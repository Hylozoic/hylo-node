...
export const convertChildrenToProjectRequests = (parentPostId, trx) => {
  return Post.query()
    ...
    .update({ is_project_request: true })
    .transacting(trx);
};
...
export const convertPostsForTagToChildren = (tagId, parentPostId, trxOpt) => {
  return Post.query((qb) => {
    ...
    qb.where(function () {
      this.where("type", "!=", "project").orWhere("type", null);
    });
  })
    .fetchAll()
    .then(({ models }) =>
      reduce(
        ...
        (promise, post) =>
          promise
            ...
            .then(() =>
              PostMembership.query((qb) =>
                qb.where("post_id", post.id)
              ).destroy(trxOpt)
            ), // it will rely on the parent posts community membership
        Promise.resolve()
      )
    );
};
      ...
      promise
        ...
        .then((postTag) =>
          bookshelf.transaction((trx) => {
            console.log(
              `attempting to migrate tag for project '${project.get("name")}'`
            );
            if (!postTag) {
              console.log(
                `tag for project '${project.get(
                  "name"
            ...
            return convertChildrenToProjectRequests(project.id, trx)
              .then(() =>
                convertPostsForTagToChildren(tagId, project.id, trxOpt)
              )
              .then(() =>
                Promise.map(
                  [PostTag, TagFollow, CommentTag, CommunityTag],
                  (model) =>
                    model.query().where("tag_id", tagId).del().transacting(trx)
                )
              )
              .then(() =>
                Tag.query((qb) => qb.where("id", tagId)).destroy(trxOpt)
              )
              .then(() => {
                console.log(
                  `tag for project '${project.get(
...
export const convertProjectPostsToChildren = () => {
  return Post.where("type", "project")
    .fetchAll()
