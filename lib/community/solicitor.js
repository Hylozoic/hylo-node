...
var queueEmail = function (emailData) {
  sails.log.debug('Queue email - ', emailData)
...
Solicitor.sendToCommunity = function (communityId) {
  ...
  .then(community => Promise.map(community.relations.users.models, user =>
...
Solicitor.sendWeekly = function () {
  ...
  .fetchAll()
  .then(communities => Promise.map(communities, community =>
