...
var unsubscribeLink = function (user) {
  return Frontend.Route.tokenLogin(user, user.generateTokenSync(), Frontend.Route.userSettings() + '?expand=solicitation')
}
...
Solicitor.sendToUser = function (user, community) {
  if (user.get('settings').email_solicitation_preference) {
