...
Digest.prototype.emailData = function(user) {
  ...
  if (!this.nonUserSpecificData) {
    this.nonUserSpecificData = {
      data: {
        ...
        commented_posts: (this.commentedPosts ? this.commentedPosts.map(function(post) {
        ...
        posts: (this.posts ? this.posts.map(this.postAttributes.bind(this)) : null),
  ...
  _.each(userData.data.commented_posts || [], function(postAttrs) {
    postAttrs.reply_url = Email.postReplyAddress(postAttrs.id, user.id);
  });

  _.each(userData.data.posts || [], function(postAttrs) {
    postAttrs.reply_url = Email.postReplyAddress(postAttrs.id, user.id);
  });
