...
var Digest = function(community, startTime, endTime) {
  ...
  sails.log.info(format("Generating digest for %s, %s to %s",
    community.get('name'), this.startTime, this.endTime));
};
...
Digest.prototype.fetchData = function(done) {
  ...
  .spread(function(users, seeds, comments) {
    sails.log.info(format('New activity: %s users, %s seeds, %s comments',
      users.length, seeds.length, comments.length));
  ...
  .then(function(comments) {
    if (!comments) return;
    var commentIds = comments.map(function(c) { return c.get('post_id') });

    return Promise.join(
      comments,
      Post.query(function(qb) {
        qb.whereIn('id', commentIds);
      }).fetchAll({withRelated: [
        {creator: function(qb) {
          qb.column('id', 'name', 'avatar_url');
        }}
...
Digest.prototype.subject = function() {
  ...
  if (this.comments) {
    names = names.concat(this.comments.map(function(comment) {
  ...
  return format("%s: %s", this.community.get('name'), summary);
};
...
Digest.prototype.seedAttributes = function(seed) {
  // fix old seed descriptions
  var description = seed.get('description');
  if (description.substring(0, 3) != '<p>')
    description = format('<p>%s</p>', description);
  ...
  return {
    ...
    reply_url:   'mailto:todo@hylo.com?subject=todo'
  }
};
...
Digest.prototype.emailData = function(user) {
    ...
    this.nonUserSpecificData = {
      data: {
        ...
        commented_seeds: (this.commentedSeeds ? this.commentedSeeds.map(function(seed) {
          return _.merge(self.seedAttributes(seed), {
            comments: seed.comments.map(function(comment) {
              var text = comment.get('comment_text');
  ...
  return _.merge(this.nonUserSpecificData, {
...
Digest.prototype.queueEmails = function() {
  ...
  return this.recipients().fetch().then(function(users) {
    sails.log.info(format('Sending to %s recipients', users.length));

    return Promise.map(users.models, function(user) {
      return Digest.queueEmail(self.emailData(user));
    })
...
Digest.prototype.sendTestEmail = function(recipient) {
  var fakeUser = {get: function() { return recipient }};
  return Email.sendCommunityDigest(this.emailData(fakeUser));
};

Digest.queueEmail = function(emailData) {
