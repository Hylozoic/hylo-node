...
Digest.prototype.fetchData = function () {
  ...
  return Promise.join(
    ...
    .fetch({withRelated: {user: userColumns}}),
    ...
    .fetchAll({withRelated: {user: userColumns}})
  )
  ...
  .tap(() => {
    sails.log.debug(format('%s: %s users, %s new posts, %s commented posts %s',
      this.communityName,
      (this.users || []).length,
      (this.posts || []).length,
...
Digest.prototype.getSharedEmailProps = function () {
  return {
    members: this.users.map(user =>
      _.merge(userAttributes(user), {
        tags: user.relations.tags.pluck('name').map(name => '#' + name)
      })),
