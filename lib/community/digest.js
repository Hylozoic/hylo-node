...
Digest.prototype.queueEmails = function() {
  ...
  return this.recipients().fetch().then(function(users) {
    ...
    return Promise.map(users.models, function(user) {
      if (self.logStore) return self.logStore.push(self.emailData(user)); // DEBUG
...
Digest.sendDaily = function() {
  var today = moment.tz('America/Los_Angeles').clone().startOf('day'),
    yesterday = today.clone().subtract(days, 'day');
...
Digest.debugSendDaily = function(logStore, concurrency, days) {
  var today = moment.tz('America/Los_Angeles').clone().startOf('day'),
    yesterday = today.clone().subtract(days, 'day');
