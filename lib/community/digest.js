...
Digest.prototype.queueEmails = function () {
  return this.recipients().fetch().then(function (users) {
    sails.log.debug(format('%s: Queueing emails for %s recipients', this.communityName, users.length))
    var queue = require('kue').createQueue()

    return Promise.map(users.models, user =>
      user.generateToken().then(token =>
        Digest.queueEmail(queue, this.emailData(user, token))))
    .then(function (enqueued) {
      sails.log.debug(format('%s: Finished queueing', this.communityName))
