...
const isOtherTag = post => !isRequest(post) && !isOffer(post) && !isEvent(post) && !isProject(post)
...
const presentPost = curry((slug, post) => pickBy(x => x, {
  ...
  requests: post.relations.children && post.relations.children.map(p => p.get('name')),
...
const formatData = curry((community, data) => {
  ...
  const findFormattedPost = id => find(p => p.id === id,
    requests.concat(offers).concat(conversations).concat(projects).concat(events).concat(postsWithNewComments))
  ...
  if (every(isEmpty, [requests, offers, conversations, events, projects])) {
