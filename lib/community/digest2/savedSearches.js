...
const presentPost = async (p, context, slug) => {
  ...
  return pickBy((x) => x, {
    ...
    when:
      post.get("start_time") || post.get("end_time")
        ? `${humanDate(post.get("start_time"))} ${
            humanDate(post.get("end_time"))
              ? `- ${humanDate(post.get("end_time"))}`
              : ""
          }`
        : undefined,
...
const shouldSendData = (data, user, type) => {
  ...
  const hasNewPosts = Object.keys(pick(postTypes, data)).some((s) =>
    postTypes.includes(s)
  );
...
const sendDigest = async (searchId, type) => {
  return await prepareDigestData(searchId)
    .then(async (data) => {
      const { lastPostId, user } = data;
      if (shouldSendData(data, user, type))
        return merge(await sendToUser(user, data), { lastPostId });
    })
    .then(async (sent = {}) => {
