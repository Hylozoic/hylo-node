...
const commaSeparate = (items, max) => {
  ...
  switch (length) {
    ...
    default:
      return `${items[0]}, ${items[1]}, and ${length - 2} other${
        length > 3 ? "s" : ""
      }`;
  }
};
...
const filterBlockedUserData = async (userId, data) => {
  ...
  for (const key of keys) {
    clonedData[key] = filter(
      (object) => !includes(get("user.id", object), blockedUserIds),
      clonedData[key]
    );
  }
...
const personalizeData = async (user, data, opts = {}) => {
  ...
  return Promise.props(
    merge(filteredData, {
      ...
      email_settings_url:
        Frontend.Route.userSettings() + clickthroughParams + "&expand=account",
