...
Serendipity.prototype.inspect = function() {
	return {
		vectors: (this.vectors || []).length,
		weights: (this.weights || []).length,
		singular: (this.singular || []).length,
		stopwords: Object.keys(this.stopwords || {}).length,
...
function readFromStream(stream, appCallback, finalCallback) {
    ...
    if (file === 'vectors') {
      var results, line_fragment;
      body = [];

      stream.on('data', function (chunk) {
        results = streamMultiFloatTable(line_fragment, chunk);
        line_fragment = results[0];
        Array.prototype.push.apply(body, results[1]);
      });
...
function streamMultiFloatTable(line_fragment, chunk) {
  var lines = (line_fragment + chunk).split('\n'),
    ret = [];

  lines.forEach(function (v, i) {
    if (!v || i === lines.length-1) return;
  ...
  return [lines[lines.length-1], ret];
}
