...
async function updateGroupMemberships ({
  ...
  async function processRow (row) {
    ...
    // BUG: need to be merging with existing settings, not overwriting them!
    // PostgreSQL has a nice operator for this... in version 9.5...

    return bookshelf.knex('group_memberships')
    .where({user_id, group_id}).update({settings})
  }
