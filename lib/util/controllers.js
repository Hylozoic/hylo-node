...
export const handleHitfinIntegrationError = (err, res) => {
  if(err.hitfinIntegrationErrorMessage) {
    res.status(err.hitfinIntegrationErrorCode)
    res.send(pick(err, 'hitfinIntegrationErrorMessage'))
    return true
  }
}

export const handleNotEnoughWalletBalanceError = (err, res) => {
  if(err.notEnoughWalletBalanceErrorMessage) {
    res.status(err.notEnoughWalletBalanceErrorCode)
    res.send(pick(err, 'notEnoughWalletBalanceErrorMessage'))
    return true
  }
}
