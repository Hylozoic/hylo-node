...
export const handleHitfinProjectSubmissionError = (err, res) => {
  if(err.hitfinProjectSubmissionErrorMessage) {
    res.status(err.hitfinProjectSubmissionErrorCode)
    res.send(pick(err, 'hitfinProjectSubmissionErrorMessage'))
    return true
  }
}
