...
import { find, flow, get, map, pick, values } from 'lodash/fp'
...
export const updateOrRemove = (table, column, fromValue, toValue, uniqueCols, knex) => {
  ...
  return knex(table).where(column, fromValue).then(rows => rowsToChange = rows)
  // find duplicate rows
...
export const countTotal = (q, table) => {
  q.select(bookshelf.knex.raw(`${table}.*, count(*) over () as total`))
}
