...
import { isNull, isUndefined, omitBy, result, toPairs } from 'lodash/fp'
...
export default class Fetcher {
  ...
  fetchRelation (relation, typename, fetchOpts, tap) {
    console.log('')
    console.log('*****')
    console.log('in fetchRelation')
    if(relation.id === 20815) {
      console.log('relation id = 20815')
    }
    ...
    if (type === 'belongsTo' && !parentFk) return Promise.resolve()
    ...
    if (type === 'hasOne' || type === 'belongsTo') {
      ...
      if(relation.relatedData.parentId === 858673) {
        console.log('HERES TE BUSINESS')
      }
