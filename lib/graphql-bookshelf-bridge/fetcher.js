...
export default class Fetcher {
  ...
  fetchOne(typename, id, idColumn = "id", args = false) {
    ...
    if (args) {
      const whitelist = mapKeys((k) => snakeCase(k), pick(["isPublic"], args));
  ...
  _getTypenameFromTableName(tableName) {
    // TODO cache this
    const matches = toPairs(this.models).filter(
      ([typename, spec]) => spec.model.collection().tableName() === tableName
    );

    if (matches.length > 1) {
      const defaultTypeForTable = matches.find(
        ([typename, spec]) => spec.isDefaultTypeForTable
      );
      if (defaultTypeForTable) return defaultTypeForTable[0];

      throw new Error(
        `tableName "${tableName}" is ambiguous: ${matches
          .map((x) => x[0])
          .join(", ")}`
      );
    }
  ...
  _loadMany(relation, { method, loader, tap, fetchOpts }) {
    return this.loaders.relations
      .load({ relation, method })
      .tap(tap)
      .then((instances) => {
        // N.B. this caching doesn't take into account data added by withPivot
        instances.each((x) => loader.clear(x.id).prime(x.id, x));
        return loader.loadMany(instances.map("id")).then((models) => {
          if (!fetchOpts.querySet) return models;
          const cleanOpts = omitBy(
            (x) => isNull(x) || isUndefined(x),
            fetchOpts
          );
