  ...
  .then(() => {
    ...
    function setupStreams (data, resolve, reject) {
      ...
      if (type === 'importPosts') {
        ...
        csvImporter.on('finish', () => resolve(csvImporter.numPostsCreated, csvImporter.errors))
        ...
        storage.on('finish', () => {
          let attachmentType = fileType && fileType.mime.split('/')[0]

          attachmentType = ['file', 'image'].includes(attachmentType)
            ? attachmentType
            : 'file'
