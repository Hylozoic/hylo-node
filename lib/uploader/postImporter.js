  ...
  return new Promise(async (resolve, reject) => {
    ...
    const postParams = {
      ...
      isPublic: record.is_public
        ? ["true", "yes"].includes(record.is_public.toLowerCase())
        : false,
...
export function createPostImporter(userId, communityId) {
  ...
  parser.on("error", (err) => {
    sails.log.error("Weird parser error, check out " + err);
  });
  ...
  parser.on("end", () => {
    ...
    promiseFactories.forEach((promiseFactory) => {
      sequence = sequence
        ...
        .catch((error) => {
          parser.errors.push(error.message ? error.message : error);
        });
    });
...
function convertMapboxToLocation(mapboxResult) {
  ...
  const neighborhoodObject =
    context && context.find((c) => c.id.includes("neighborhood"));
  const postcodeObject =
    context && context.find((c) => c.id.includes("postcode"));
  ...
  const countryObject =
    context && context.find((c) => c.id.includes("country"));
  ...
  return {
    ...
    bbox: mapboxResult.bbox
      ? [
          { lng: mapboxResult.bbox[0], lat: mapboxResult.bbox[1] },
          { lng: mapboxResult.bbox[2], lat: mapboxResult.bbox[3] },
        ]
      : null,
