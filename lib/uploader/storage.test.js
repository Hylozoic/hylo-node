...
describe("Uploader.storage.makePath", () => {
  ...
  it("stores a file based on the user who uploaded it", () => {
    ...
    expect(makePath("communityAvatar", 17, { userId: 41, fileType })).to.match(
      /all-the-things\/user\/41\/communityAvatar\/17\/\d{13}_\d{4}\.png/
    );
  });
  ...
  it("uses an existing filename if present", () => {
    expect(
      makePath("post", 17, {
        userId: 41,
        filename: "http://wow.com/foo.pdf?bar=1",
      })
    ).to.equal("all-the-things/user/41/post/17/foo.pdf");
  });

  it("uses default values", () => {
    expect(makePath("post", null, { filename: "foo.pdf" })).to.equal(
      "all-the-things/user/system/post/new/foo.pdf"
    );
  });
});
...
describe("Uploader.storage.createS3StorageStream", () => {
  ...
  it('allows listening for a "progress" event', () => {
    ...
    expect(stream.upload.on).to.have.been.called.with(
      "httpUploadProgress",
      listener
    );
  });
