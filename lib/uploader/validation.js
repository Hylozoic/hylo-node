...
async function hasPermission(userId, type, id) {
  if (type.startsWith("user")) {
    ...
    return Promise.reject(
      new Error(
        "Validation error: Not allowed to change settings for another person"
      )
    );
  }
  ...
  if (type.startsWith("network")) {
    return NetworkMembership.hasModeratorRole(userId, id).then((ok) => {
      if (!ok)
        throw new Error("Validation error: Not a moderator of this network");
    });
  }
  ...
  if (type.startsWith("post")) {
    ...
    return Post.find(id).then((post) => {
      if (!post || post.get("user_id") !== userId)
