...
const isDiscussion = post => !isRequest(post) && !isOffer(post) && !isEvent(post) && !isProject(post) && !isResource(post) && !isChat(post)
...
const formatData = curry((community, data) => {
  ...
  const findFormattedPost = id => find(p => p.id === id,
    requests.concat(offers).concat(discussions).concat(projects).concat(events).concat(resources).concat(postsWithNewComments))
  ...
  if (every(isEmpty, [requests, offers, resources, discussions, events, projects])) {
