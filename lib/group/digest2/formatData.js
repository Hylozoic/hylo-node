...
const formatData = curry((group, data) => {
  ...
  const findFormattedPost = id => find(p => p.id === id,
    requests.concat(offers).concat(discussions).concat(projects).concat(events).concat(resources).concat(chats).concat(postsWithNewComments))
  ...
  if (every(isEmpty, [requests, offers, resources, discussions, events, projects, topicsWithChats, postsWithNewComments])) {
