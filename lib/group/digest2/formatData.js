...
const isDiscussion = post => !isRequest(post) && !isOffer(post) && !isEvent(post) && !isProject(post) && !isResource(post)
...
const presentPost = curry((slug, post) => {
  ...
  return pickBy(x => x, {
    id: post.id,
    title: post.get('name'),
    details: RichText.qualifyLinks(post.details(), null, null, slug),
...
const presentComment = curry((slug, comment) => ({
  id: comment.id,
  text: RichText.qualifyLinks(comment.text(), null, null, slug),
...
const formatData = curry((community, data) => {
  ...
  const conversations = map(presentPost(slug), filter(isDiscussion, data.posts))
