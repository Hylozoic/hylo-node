...
exports.up = function (knex, Promise) {
  return knex.schema
    ...
    .then(() =>
      knex.schema.createTable("group_connections", (t) => {
        t.bigIncrements().primary();
        t.bigInteger("parent_group_id")
          .references("id")
          .inTable("groups")
          .notNullable();
        t.integer("parent_group_data_type").notNullable();
        t.bigInteger("child_group_id")
          .references("id")
          .inTable("groups")
          .notNullable();
    ...
    .then(() =>
      setDeferrable(
        knex,
        "group_connections",
        "group_connections_child_group_id_foreign"
      )
    )
    .then(() =>
      setDeferrable(
        knex,
        "group_connections",
        "group_connections_parent_group_id_foreign"
      )
    )
    .then(() =>
      knex.schema.createTable("group_memberships", (t) => {
        t.bigIncrements().primary();
    ...
    .then(() =>
      setDeferrable(
        knex,
        "group_memberships",
        "group_memberships_group_id_foreign"
      )
    );
};
