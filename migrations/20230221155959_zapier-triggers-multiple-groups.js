...
exports.up = async function (knex) {
  await knex.schema.createTable('zapier_triggers_groups', table => {
    ...
    table.bigInteger('trigger_id').references('id').inTable('zapier_triggers')
    table.bigInteger('group_id').references('id').inTable('groups')
    table.index(['trigger_id'])
  })

  await knex.raw('alter table zapier_triggers_groups alter constraint zapier_triggers_groups_trigger_id_foreign deferrable initially deferred')
  ...
  await knex.raw('insert into zapier_triggers_groups (trigger_id, group_id) (select id, group_id from zapier_triggers)')
