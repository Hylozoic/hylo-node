...
describe('Skill', function() {
  ...
  before(function() {
    cat = new User({name: 'Cat', email: 'a@skill.c'});
    otherCat = new User({name: 'Other Cat', email: 'b@skill.c'});
  ...
  describe('#update', function() {
    ...
    it('adds and removes skills for one user', function () {
      return Skill.update(['clawing', 'sleeping', 'pouncing'], cat.id)
      .then(function() {
    ...
    it('makes no changes if the list is the same', function () {
      ...
      return Skill.update(['meowing', 'clawing'], cat.id)
      .then(function() {
        return cat.load('skills');
      })
      .then(function() {
