...
describe("Invitation", function () {
  ...
  describe("#use", function () {
    let user, community, tag, invitation1, invitation2, inviter;
    ...
    it("creates a membership and marks itself used", async () => {
      await bookshelf.transaction((trx) =>
        invitation1.use(user.id, { transacting: trx })
      );
      expect(invitation1.get("used_by_id")).to.equal(user.id);
      expect(invitation1.get("used_at").getTime()).to.be.closeTo(
        new Date().getTime(),
        2000
      );
    ...
    it("creates a tag_follow when it has a tag_id", function () {
      return bookshelf
        .transaction((trx) => invitation2.use(user.id, { transacting: trx }))
        .then(
          TagFollow.where({
            user_id: user.id,
  ...
  describe("createAndSend", () => {
    let community, user, inviter, invEmail, invData;
    before(() => {
      community = factories.community();
      user = factories.user();
      inviter = factories.user();
    ...
    it("creates an invite and calls Email.sendInvitation", async () => {
      ...
      return Invitation.createAndSend({ invitation })
        .then(() =>
          Invitation.where({ email: email, community_id: community.id }).fetch()
        )
        .then((invitation) => {
  ...
  describe(".resendAllReady", () => {
    ...
    before(() => {
      ...
      return Promise.join(
        ...
        return Promise.map(
          attributes,
          ({ email, sent_count, last_sent_at, used_by_id }) =>
            Invitation.create({
              communityId: community.id,
              userId,
              email,
            }).then((i) =>
              i.save({ sent_count, last_sent_at, used_by_id }, { patch: true })
            )
        );
      });
    });
