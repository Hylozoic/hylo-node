...
describe("Tag", () => {
  ...
  describe("updateForPost", () => {
    it("creates a tag from topicNames param", () => {
      ...
      return post
        ...
        .then((tag) => {
          ...
          expect(tag.relations.posts.models[0].get("name")).to.equal(
            "New Tagged Post"
          );
        });
    });
    ...
    it("ignores duplicate tags", () => {
      ...
      return new Tag({ name: "duplicated" })
        .save()
        .then(() => post.save())
        .then((post) => Tag.updateForPost(post, ["duplicated", "duplicated"]))
        .then(() =>
          Tag.find({ name: "duplicated" }, { withRelated: ["posts"] })
        )
        .then((tag) => {
          expect(tag).to.exist;
    ...
    it("removes a tag", () => {
      ...
      const tag = new Tag({ name: "newtagfive" });
        ...
        .then(() =>
          Promise.join(
            ...
            (removed, added) => {
              ...
              expect(added.relations.posts.models[0].get("name")).to.equal(
                "New Tagged Post Five"
              );
            }
          )
        );
    ...
    it("associates tags with communities of which the user is a member", () => {
      ...
      return Promise.join(post.save(), c2.save())
        ...
        .then(() =>
          Tag.find({ name: "newtagnine" }, { withRelated: ["communities"] })
        )
        .then((tag) => {
          expect(tag).to.exist;
          expect(tag.get("name")).to.equal("newtagnine");
          expect(tag.relations.communities.length).to.equal(1);
          const communities = sortBy(tag.relations.communities.models, (c) =>
            c.get("name")
          );
    ...
    it("preserves existing tag owner", () => {
      ...
      const owner = factories.user();
      const post = new Post({
        name: "New Tagged Post",
        description: "no tags in the body",
      });
      const tag = new Tag({ name: "newtagten" });
      const c2 = factories.community({ name: "Community Four" });
      return user
        .save()
        ...
        .then(() =>
          Tag.find({ name: "newtagten" }, { withRelated: ["communities"] })
        )
        .then((tag) => {
          expect(tag).to.exist;
          expect(tag.get("name")).to.equal("newtagten");
          expect(tag.relations.communities.length).to.equal(1);
          const communities = sortBy(tag.relations.communities.models, "id");
          expect(communities[0].get("name")).to.equal(c1.get("name"));
  ...
  describe(".merge", () => {
    ...
    it("removes rows that would cause duplicates and updates the rest", function () {
      return Tag.merge(t1.id, t2.id)
        ...
        .then(() => {
          expect(t1.relations.posts.map("id").sort()).to.deep.equal(
            [p1.id, p2.id].sort()
          );
        ...
        .then(() => p2.load("tags"))
        .then(() => {
          expect(p2.relations.tags.map("id").sort()).to.deep.equal(
            [t1.id, t3.id].sort()
          );
        });
    });
  ...
  describe(".taggedPostCount", () => {
    ...
    beforeEach(() => {
      ...
      return Promise.all([t1, p1, p2, c].map((x) => x.save())).then(() =>
        Promise.all([
          k("posts_tags").insert({ tag_id: t1.id, post_id: p1.id }),
    ...
    it("doesn't count inactive posts in the count", function () {
      return Tag.taggedPostCount(t1.id).then((count) => {
  ...
  describe(".followersCount", () => {
    ...
    beforeEach(() => {
      ...
      return Promise.all([t1, c1, c2].map((x) => x.save())).then(() =>
        Promise.all([
          k("communities_tags").insert({ tag_id: t1.id, community_id: c.id }),
          k("tag_follows").insert({
            tag_id: t1.id,
    ...
    it("correctly counts the number of followers across the whole site", function () {
      return Tag.followersCount(t1.id).then((count) => {
        expect(count).to.equal(1);
      });
    });

    it("correctly counts the number of followers for a given community", function () {
      return Tag.followersCount(t1.id, { communityId: c2.id }).then((count) => {
  ...
  describe(".nonexistent", () => {
    ...
    it("returns a map of names to the communities they are missing from, filtered by a user's memberships", () => {
      return Tag.nonexistent(["tag1", "tag2", "tag3"], [cx.id, cy.id]).then(
        (results) => {
