...
describe("sendToCommunities", () => {
  ...
  before(() => {
    ...
    sendToCommunities = mockRequire.reRequire(
      "../../../../api/models/flaggedItem/notifyUtils"
    ).sendToCommunities;
  ...
  it("sends a message from axolotl to the community moderators", () => {
    ...
    return sendToCommunities(flaggedItem, communities).then((result) => {
  ...
  it("sends illegal content to HYLO ADMINS as well", () => {
    ...
    const expectedText = [`${message} ${c1.id}`, `${message} ${c2.id}`];

    const hyloAdminIds = process.env.HYLO_ADMINS.split(",").map((id) =>
      Number(id)
    );
...
describe("notifying moderators", () => {
  let argUserIds, argText, flaggedItem, modIds1, modIds2;
  ...
  it("works for a post", () => {
    const notifyModeratorsPost = mockRequire.reRequire(notifyUtilsPath)
      .notifyModeratorsPost;
    return notifyModeratorsPost(flaggedItem).then((result) => {
      expect(argUserIds).to.deep.equal([modIds1, modIds2]);
      expect(argText).to.deep.equal(["the message 1", "the message 2"]);
    });
  });

  it("works for a comment", () => {
    const notifyModeratorsComment = mockRequire.reRequire(notifyUtilsPath)
      .notifyModeratorsComment;
    return notifyModeratorsComment(flaggedItem).then((result) => {
      expect(argUserIds).to.deep.equal([modIds1, modIds2]);
      expect(argText).to.deep.equal(["the message 1", "the message 2"]);
    });
  });
  ...
  it("works for a member", () => {
    const notifyModeratorsMember = mockRequire.reRequire(notifyUtilsPath)
      .notifyModeratorsMember;
    return notifyModeratorsMember(flaggedItem).then((result) => {
      expect(argUserIds).to.deep.equal([modIds1, modIds2]);
      expect(argText).to.deep.equal(["the message 1", "the message 2"]);
    });
  });
