...
const destroyAllPushNotifications = () => {
  return PushNotification.fetchAll().then((pns) =>
    pns.map((pn) => pn.destroy())
  );
};
...
describe("Notification", function () {
  ...
  before(() => {
    return factories
      .user({ avatar_url: "http://joe.com/headshot.jpg", name: "Joe" })
      .save()
      .then((u) => {
        actor = u;
      })
      ...
      .then(() =>
        new Comment({ text: "hi", user_id: actor.id, post_id: post.id }).save()
      )
      .then((c) => {
        comment = c;
      })
      .then(() =>
        factories
          .community({ name: "My Community", slug: "my-community" })
          .save()
      )
      .then((c) => {
        community = c;
  ...
  describe(".send", () => {
    ...
    it("sends a push for a new post", () => {
      return preloadNotification(activities.newPost, Notification.MEDIUM.Push)
        ...
        .then((pns) => {
          ...
          const pn = pns.first();
          expect(pn.get("alert")).to.equal(
            'Joe posted "My Post" in My Community'
          );
        });
    });
    ...
    describe("with a user with push notifications for comments enabled", () => {
      it("sends no push for a comment", () => {
        return preloadNotification(
          activities.newComment,
          Notification.MEDIUM.Push
        )
          .then((notification) => notification.send())
          .then(() =>
            PushNotification.where({ device_id: device.id }).fetchAll()
          )
    ...
    describe("to a user with push notifications for comments enabled", () => {
      beforeEach(() =>
        reader.addSetting({ comment_notifications: "push" }, true)
      );
      ...
      it("sends a push for a comment", () => {
        return preloadNotification(
          ...
          .then((pns) => {
            ...
            const pn = pns.first();
            expect(pn.get("alert")).to.equal(
              `Joe: "${comment.get("text")}" (in "My Post")`
            );
          });
      });
      ...
      it("sends a push for a mention in a comment", () => {
        return preloadNotification(
          ...
          .then((pns) => {
            ...
            const pn = pns.first();
            expect(pn.get("alert")).to.equal(
              'Joe mentioned you: "hi" (in "My Post")'
            );
          });
      });
    });
    ...
    it("sends a push for an approved join request", () => {
      return preloadNotification(
        activities.approvedJoinRequest,
        Notification.MEDIUM.Push
      )
        .then((notification) => notification.send())
        .then(() => PushNotification.where({ device_id: device.id }).fetchAll())
        .then((pns) => {
          ...
          const pn = pns.first();
          expect(pn.get("alert")).to.equal(
            "Joe approved your request to join My Community"
          );
        });
    });
      ...
      return Promise.join(
        ...
        .then((notifications) => {
          ...
          expect(notifications.pluck("medium").sort()).to.deep.equal(
            [Notification.MEDIUM.Push, Notification.MEDIUM.InApp].sort()
          );
        });
    });
  });
  ...
  describe("sendCommentNotificationEmail", () => {
    let args, community;
    beforeEach(() => {
      spyify(Email, "sendNewCommentNotification", (x) => {
        args = x;
      });
    ...
    it("sets the correct email attributes", () => {
      ...
      return note.sendCommentNotificationEmail().then(() => {
  ...
  describe("sendPushAnnouncement", () => {
    let post, notification, reader, community, activity, user, alertText, path;
    ...
    before(async () => {
      ...
      activity = await factories
        .activity({ post_id: post.id, reader_id: reader.id })
        .save();
      notification = await factories
      ...
      notification.relations.activity.relations.reader.sendPushNotification = spy(
        (inAlertText, inPath) => {
    ...
    it("calls sendPushNotification with the correct params", async () => {
      ...
      expect(
        notification.relations.activity.relations.reader.sendPushNotification
      ).to.have.been.called();
      expect(alertText).to.equal(PushNotification.textForAnnouncement(post));
      expect(path).to.equal(
        url.parse(Frontend.Route.post(post, community)).path
      );
    });
  });
});
