  ...
  describe('.find', function () {
    before(function () {
      ...
      return setup.clearDb().then(function () {
        ...
        post.communities().attach(community)
      })
    })

    it('works with a community id', function () {
      return PostMembership.find(post.id, community.id).then(function (membership) {
        expect(membership).to.exist
      })
    })

    it('works with a community slug', function () {
      return PostMembership.find(post.id, community.get('slug')).then(function (membership) {
        expect(membership).to.exist
      })
    })

    it('returns nothing for a blank post id', function () {
      return PostMembership.find(null, community.id).then(function (membership) {
