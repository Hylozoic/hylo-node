const setup = require(require("root-path")("test/setup"));
...
describe("PostMembership", function () {
  ...
  describe(".find", function () {
    ...
    it("works with a community id", function () {
      return PostMembership.find(post.id, community.id).then(
        (membership) => expect(membership).to.exist
      );
    });

    it("works with a community slug", function () {
      return PostMembership.find(post.id, community.get("slug")).then(
        (membership) => expect(membership).to.exist
      );
    });

    it("returns nothing for a blank post id", function () {
      return PostMembership.find(null, community.id).then(
        (membership) => expect(membership).not.to.exist
      );
    });
  });
});
