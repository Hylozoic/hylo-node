...
describe("BlockedUser", () => {
  ...
  describe("create", () => {
    it("throws if other_user_id is user_id", () => {
      expect(() => BlockedUser.create("1", "1")).to.throw(
        /blocked_user_id cannot equal user_id/
      );
    });

    it("throws if user_id is null", () => {
      expect(() => BlockedUser.create(null, "1")).to.throw(
        /must provice a user_id and blocked_user_id/
      );
    });
  ...
  describe("find", () => {
    it("throws if user_id is missing", () => {
      expect(() => BlockedUser.find()).to.throw(
    ...
    it("finds a BlockedUser if a match exists", () => {
      ...
      return c
        ...
        .then((blockedUser) =>
          expect(blockedUser.get("id")).to.equal(c.get("id"))
        );
    });
  });
});
