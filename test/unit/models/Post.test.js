...
describe("Post", function () {
  it("getDetailsText", async () => {
    ...
    const post = await factories
      .post({
        description:
          "<p>hello <a data-user-id=\"334\" data-entity-type='mention'>John Doe</a> #MOO</p>",
      })
      .save();
  ...
  describe("#addFollowers", function () {
    ...
    it("queries for lastReadAt correctly", async () => {
      ...
      expect((await post.lastReadAtForUser(u1.id)).getTime()).to.be.closeTo(
        new Date(0).getTime(),
        2000
      );
  ...
  describe("#getCommenters", function () {
    let u1, u2, u3, u4, u5, u6, u7, u8, post;
  ...
  describe("#isVisibleToUser", () => {
    ...
    it("is true if the post is public", () => {
      return post
        .save({ visibility: Post.Visibility.PUBLIC_READABLE }, { patch: true })
    ...
    it("is false if the user is not connected by community", () => {
      return Post.isVisibleToUser(post.id, user.id).then(
        (visible) => expect(visible).to.be.false
      );
    });

    it("is true if the user and post share a community", () => {
      return c2
        .addGroupMembers([user.id])
        .then(() => Post.isVisibleToUser(post.id, user.id))
        .then((visible) => expect(visible).to.be.true);
    });
  ...
  describe(".createActivities", () => {
    ...
    it("creates activity for community members", () => {
      ...
      return post
        ...
        .then((activities) => {
          ...
          expect(activities.pluck("reader_id").sort()).to.deep.equal(
            [u2.id, u3.id].sort()
          );
          activities.forEach((activity) => {
            expect(activity.get("actor_id")).to.equal(u.id);
            expect(activity.get("meta")).to.deep.equal({
              reasons: [`newPost: ${c.id}`],
            });
    ...
    it("creates an activity for a mention", () => {
      ...
      return post
        ...
        .then(() => post.createActivities())
        .then(() =>
          Activity.where({ post_id: post.id, reader_id: u3.id }).fetchAll()
        )
        .then((activities) => {
    ...
    it("creates an activity for a tag follower", () => {
      ...
      return new Tag({ name: "FollowThisTag" })
        .save()
        .tap((tag) =>
          u3.followedTags().attach({ tag_id: tag.id, community_id: c.id })
        )
        .then(() => post.save())
        ...
        .then((activities) => {
          ...
          expect(activity.get("actor_id")).to.equal(u.id);
          expect(activity.get("meta")).to.deep.equal({
            reasons: [`newPost: ${c.id}`, "tag: FollowThisTag"],
          });
          expect(activity.get("unread")).to.equal(true);
        });
    });
  ...
  describe("#updateFromNewComment", () => {
    ...
    it("updates several attributes", async () => {
      ...
      await comment.save();
      await Post.updateFromNewComment({
        postId: post.id,
