...
describe("Comment", () => {
  describe("cleanEmailText", () => {
    it("wraps content in <p> tags and handles weird newlines", () => {
      ...
      expect(Comment.cleanEmailText(user, text)).to.equal(
        "<p>Ok then<br>All right</p>\n<p>So it shall be</p>\n"
      );
    });
    ...
    it("removes a common signature pattern with two dashes", () => {
      ...
      expect(Comment.cleanEmailText(user2, text)).to.equal(
        "<p>Let&#39;s do it.</p>\n"
      );
    });
  ...
  describe("sendDigests", () => {
    let u1, u2, post, comments, log, now;
    ...
    beforeEach(async () => {
      ...
      u1 = factories.user({
        avatar_url: "foo.png",
        settings: { dm_notifications: "both" },
      });
      u2 = factories.user({
        avatar_url: "bar.png",
        settings: { dm_notifications: "both" },
      ...
      await RedisClient.create().delAsync(
        Comment.sendDigests.REDIS_TIMESTAMP_KEY
      );
    });
    ...
    describe("with a message thread", () => {
      ...
      it("sends a digest of recent messages", () => {
        return Comment.sendDigests().then((count) => {
          ...
          expect(send2.data.messages).to.deep.equal([
            {
      ...
      it("respects lastReadAt", async () => {
        ...
        return Comment.sendDigests().then((count) => {
          ...
          expect(log[0].data.messages).to.deep.equal([
            {
              text: comments[3].get("text"),
              name: u2.get("name"),
              avatar_url: u2.get("avatar_url"),
      ...
      it("respects dm_notifications setting", () => {
        return u1
          .save({ settings: { dm_notifications: "push" } })
