...
const defaultAvatarUrl =
  "http://hylo-app.s3.amazonaws.com/misc/default_community_avatar.png";

describe("AssetManagement", () => {
  let community, origBucket;
  ...
  describe("copyAsset", () => {
    it("throws an error if misconfigured", function () {
      const promise = AssetManagement.copyAsset(
        community,
        "community",
        "avatar_url"
      );
  ...
  describe("resizeAsset", () => {
    it("throws an error if misconfigured", function () {
      const promise = AssetManagement.resizeAsset(community, "avatar_url", {
        width: 200,
        height: 200,
      });
      return expect(promise).to.eventually.be.rejected;
