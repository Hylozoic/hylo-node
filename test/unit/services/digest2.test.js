...
import {
  ...
  shouldSendData,
  getRecipients,
} from "../../../lib/community/digest2/util";
...
describe("community digest v2", () => {
      ...
      const data = {
        ...
        posts: [
          model({
            ...
            relations: {
              selectedTags: collection([model({ name: "request" })]),
              user: u1,
            },
          }),
          model({
          ...
          model({
            ...
            name: "I have cookies!",
            relations: {
              selectedTags: collection([model({ name: "offer" })]),
              user: u2,
            },
          }),
          model({
            ...
            relations: {
              selectedTags: collection([model({ name: "other" })]),
              user: u2,
            },
          }),
          model({
        ...
        requests: [
          {
            ...
            title: "Foo!",
            details:
              `<p><a href="${prefix}/u/21">Edward West</a> &amp; ` +
        ...
        conversations: [
          {
            ...
            user: u3.attributes,
            details:
              '<p><a href="mailto:foo@bar.com">foo@bar.com</a> and ' +
            ...
            comments: [
              { id: 3, user: user.pick("id", "avatar_url"), text: "Na" },
              {
                id: 4,
                user: u2.attributes,
                text: `Woa <a href="${prefix}/u/4">Bob</a>`,
              },
            ],
      ...
      return personalizeData(user, data).then((newData) => {
        ...
        expect(newData).to.deep.equal(
          merge({}, data, {
            ...
            conversations: [
              {
                ...
                details:
                  '<p><a href="mailto:foo@bar.com">foo@bar.com</a> and ' +
                  `<a href="${prefix}/u/2?ya=1${ctParams.replace(
                    "?",
                    "&"
                  )}">Person</a></p>`,
                ...
                comments: [
                  { id: 3, user: user.pick("id", "avatar_url"), text: "Na" },
                  {
                    id: 4,
                    user: u2.attributes,
                    text: `Woa <a href="${prefix}/u/4${ctParams}">Bob</a>`,
                  },
                ],
              },
            ],
            ...
            email_settings_url:
              Frontend.Route.userSettings() + ctParams + "&expand=account",
  ...
  describe("shouldSendData", () => {
    ...
    describe("when the community's post_prompt_day is today", () => {
      ...
      beforeEach(() => {
        ...
        community.addSetting({
          post_prompt_day: moment.tz(defaultTimezone).day(),
        });
        return community.save();
      });

      it("is false -- feature disabled", () =>
        shouldSendData({}, community.id).then(
    ...
    describe("when the community's post_prompt_day is not today", () => {
      ...
      beforeEach(() => {
        community = factories.community();
        community.addSetting({
          post_prompt_day: moment.tz(defaultTimezone).day() + 1,
        });
        return community.save();
      });
  ...
  describe("sendAllDigests", () => {
    let args, u1, u2, community, post;

    before(async () => {
      spyify(Email, "sendSimpleEmail", function () {
        args = arguments;
      });
    ...
    it("calls SendWithUs with expected data", function () {
      this.timeout(10000);
      const clickthroughParams = `?ctt=digest_email&cti=${
        u1.id
      }&ctcn=${encodeURIComponent(community.get("name"))}`;
      ...
      return sendAllDigests("daily").then((result) => {
        ...
        expect(args[2]).to.deep.equal({
          ...
          community_url:
            Frontend.Route.community(community) + clickthroughParams,
  ...
  describe("sendDigest", () => {
    ...
    describe("when there is no data and post_prompt_day matches", () => {
      ...
      it("does not send -- feature disabled", () => {
        return sendDigest(community.id, "daily").then((result) =>
    ...
    describe("when there is no data and post_prompt_day does not match", () => {
      it("does not send", () => {
        return sendDigest(community.id, "daily").then(
...
describe("getRecipients", () => {
  let c, uIn1, uOut1, uOut2, uOut3, uOut4, uOut5, uIn2;
  ...
  it("only returns active members with email turned on and the right digest type", () => {
    return getRecipients(c.id, "daily").then((models) => {
      expect(models.length).to.equal(2);
      expect(models.map((m) => m.id).sort()).to.deep.equal(
        [uIn1.id, uIn2.id].sort()
      );
    });
  });
});
