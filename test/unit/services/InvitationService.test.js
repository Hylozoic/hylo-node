...
describe("InvitationService", () => {
  let community, inviter, invitee, invitation;
  ...
  describe("check", () => {
    ...
    it("should find a community by a valid accessCode", () => {
      return InvitationService.check(
        invitee.get("id"),
        null,
        community.get("beta_access_code")
      ).then((result) => expect(result.valid).to.equal(true));
    });

    it("should find a community by a valid token", () => {
      const userId = invitee.get("id");
      const token = invitation.get("token");
      return InvitationService.check(userId, token, null).then((result) =>
  ...
  describe("use", () => {
    ...
    it("should join the invitee to community if beta_access_code is valid", () => {
      const accessCode = community.get("beta_access_code");
      return InvitationService.use(invitee.get("id"), null, accessCode).then(
        (membership) =>
    ...
    it("should join the invitee to community if token is valid", () => {
      const userId = invitee.get("id");
      const token = invitation.get("token");
      return InvitationService.use(userId, token, null).then((membership) =>
        expect(membership.attributes).to.contain({
          user_id: invitee.get("id"),
          active: true,
        })
    ...
    it("should join the invitee to community by accessCode if both an accessCode and token are provided", () => {
      ...
      const token = invitation.get("token");
      const accessCode = community.get("beta_access_code");
      return InvitationService.use(userId, token, accessCode).then(
        (membership) => {
          return invitation.refresh().then((updatedInvitation) => {
            expect(updatedInvitation.get("used_by_id")).to.equal(
              invitee.get("id")
            );
