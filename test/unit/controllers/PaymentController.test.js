...
describe("PaymentController", () => {
  let req, res, registerStripeAccount, PaymentController;
  const userId = 1234;
  const code = "abcd";
  ...
  before(() => {
    ...
    PaymentController = mock.reRequire(
  ...
  describe("#registerStripe", () => {
    it("calls registerStripeAccount and redirects", () => {
      return PaymentController.registerStripe(req, res).then(() => {
        expect(registerStripeAccount).to.have.been.called.with(userId, code);
        expect(res.redirect).to.have.been.called.with(
