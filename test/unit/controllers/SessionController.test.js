...
describe("SessionController.findUser", () => {
  let u1, u2;
  const findUser = SessionController.findUser;
  ...
  describe("with no directly linked user", () => {
    it("picks a user with matching email address", () => {
      return findUser("facebook", u2.get("email"), "foo").then((user) => {
  ...
  describe("with a directly linked user", () => {
    before(() => {
      return LinkedAccount.create(u1.id, {
        type: "facebook",
        profile: { id: "foo" },
      });
    });
...
describe("SessionController", function () {
  ...
  describe(".createWithToken", () => {
    ...
    it("logs a user in and redirects (Web/GET request)", () => {
      ...
      return SessionController.createWithToken(req, res).then(() => {
  ...
  describe(".finishFacebookOAuth", () => {
    let req, res, origPassportAuthenticate;
    ...
    const expectMatchMockProfile = (userId) => {
      return User.find(userId, { withRelated: "linkedAccounts" }).then(
        (user) => {
          const account = user.relations.linkedAccounts.first();
          ...
          expect(user.get("avatar_url")).to.equal(
            "https://graph.facebook.com/100101/picture?type=large"
          );
          return user;
        }
      );
      ...
      return SessionController.finishFacebookOAuth(req, res)
        ...
        .then((user) => {
          ...
          expect(user.get("facebook_url")).to.equal(
            "http://www.facebook.com/100101"
          );
          const account = user.relations.linkedAccounts.find(
            (a) => a.get("provider_key") === "facebook"
          );
          expect(account).to.exist;
        });
    });
    ...
    describe("with no email in the auth response", () => {
      beforeEach(() => {
        const profile = _.merge(_.cloneDeep(mockProfile), {
          email: null,
          emails: null,
        });
        passport.authenticate = spy((strategy, callback) => () =>
          callback(null, profile)
        );
      });

      it("sets an error in the view parameters", () => {
        return SessionController.finishFacebookOAuth(req, res).then(() => {
    ...
    describe("for an existing user", () => {
      ...
      it("creates a new linked account", () => {
        return SessionController.finishFacebookOAuth(req, res).then(() =>
          expectMatchMockProfile(user.id)
        );
      });

      describe("with an existing Facebook account", () => {
        beforeEach(() =>
          LinkedAccount.create(user.id, {
            type: "facebook",
            profile: { id: "foo" },
          })
        );
      ...
      describe("with a linked account that belongs to a different user", () => {
        ...
        it("changes ownership", () => {
          return SessionController.finishFacebookOAuth(req, res)
            ...
            .then((user) =>
              expect(user.relations.linkedAccounts.first().id).to.equal(
                account.id
              )
            );
        });
