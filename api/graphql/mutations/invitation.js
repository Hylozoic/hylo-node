...
export async function createInvitation(userId, communityId, data) {
  ...
  return GroupMembership.hasModeratorRole(userId, community)
    .then((ok) => {
      if (!ok)
        throw new Error(
          "You don't have permission to create an invitation for this community"
        );
    })
    .then(() => Community.find(communityId))
    .then((community) => {
      if (!community)
        throw new Error("Cannot find community to send invites for");
...
export function expireInvitation(userId, invitationId) {
  return InvitationService.checkPermission(userId, invitationId)
    .then((ok) => {
      if (!ok)
        throw new Error("You don't have permission to modify this invitation");
    })
...
export async function reinviteAll(userId, communityId) {
  ...
  return GroupMembership.hasModeratorRole(userId, community)
    ...
    .then(() =>
      InvitationService.reinviteAll({ sessionUserId: userId, communityId })
    )
    .then(() => ({ success: true }));
}
