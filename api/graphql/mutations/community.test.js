...
describe("moderation", () => {
  ...
  before(function () {
    ...
    return Promise.join(community.save(), user.save()).then(() =>
      user.joinCommunity(community, GroupMembership.Role.MODERATOR)
    );
  });
  ...
  describe("removeModerator", () => {
    ...
    it("throws an error if youre not a moderator", async () => {
      ...
      await nonModeratorUser.joinCommunity(
        community,
        GroupMembership.Role.DEFAULT
      );
      ...
      return expect(
        removeModerator(nonModeratorUser.id, user2.id, community.id, true)
...
describe("createCommunity", () => {
  ...
  before(async () => {
    ...
    starterCommunity = await factories
      .community({ slug: "starter-posts" })
      .save();
...
describe("deleteCommunityTopic", () => {
  ...
  it("deletes the topic", async () => {
    ...
    const searched = await CommunityTag.where({
