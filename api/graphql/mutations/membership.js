...
export async function updateMembership(
  userId,
  { communityId, data, data: { settings } }
) {
  const whitelist = mapKeys(pick(data, ["newPostCount"]), (v, k) =>
    snakeCase(k)
  );
  ...
  const membership = await GroupMembership.forIds(
    userId,
    communityId,
    Community
  ).fetch();
  if (!membership)
    throw new Error(
      "Couldn't find membership for community with id",
      communityId
    );
