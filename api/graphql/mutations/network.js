...
export async function networkMutationPermissionCheck(
...
export async function addCommunityToNetwork(authZ, { communityId, networkId }) {
  ...
  await Community.where("id", communityId).save("network_id", networkId, {
    method: "update",
    patch: true,
  });
...
export async function addNetworkModeratorRole(authZ, { personId, networkId }) {
  await networkMutationPermissionCheck(authZ, networkId);
  const hasModeratorRole = await NetworkMembership.hasModeratorRole(
    personId,
    networkId
  );
  if (hasModeratorRole) {
    throw new Error(
      "That user already has moderator permissions for that network."
    );
  }
...
export async function removeCommunityFromNetwork(
  authZ,
  { communityId, networkId }
) {
  await networkMutationPermissionCheck(authZ, networkId);
  await Community.where("id", communityId).save("network_id", null, {
    method: "update",
    patch: true,
  });
  return Network.find(networkId, { withRelated: ["communities"] });
...
export async function removeNetworkModeratorRole(
  authZ,
  { personId, networkId }
) {
  await networkMutationPermissionCheck(authZ, networkId);
  await NetworkMembership.where({
    user_id: personId,
