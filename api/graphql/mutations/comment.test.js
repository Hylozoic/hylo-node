...
describe("validateCommentCreateData", () => {
	...
	before(function () {
		...
		return Promise.join(post.save(), post2.save(), user.save()).then(
			function () {
	...
	it("resolves if checks pass", () => {
		...
		return validateCommentCreateData(user.id, data).catch(() =>
...
describe("canDeleteComment", () => {
	...
	it("allows the creator to delete", () => {
		return canDeleteComment(u1.id, c).then((canDelete) => {
			expect(canDelete).to.be.true;
		});
	});

	it("allows a moderator of one of the comments communities to delete", () => {
		return canDeleteComment(u2.id, c).then((canDelete) => {
			expect(canDelete).to.be.true;
		});
	});

	it("does not allow anyone else to delete", () => {
		return canDeleteComment(u3.id, c).then((canDelete) => {
