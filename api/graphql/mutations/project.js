...
export function createProject(userId, data) {
  ...
  const memberIds = data.memberIds ? uniq(data.memberIds.concat([userId])) : [];
...
export async function addPeopleToProjectRole(userId, peopleIds, projectRoleId) {
  const projectRole = await ProjectRole.find(projectRoleId, {
    withRelated: "project",
  });
  ...
  const project = await getModeratedProject(
    userId,
    projectRole.relations.project.id
  );
  ...
  const checkForSharedCommunity = (id) =>
    Group.inSameGroup([userId, id], Community).then((doesShare) => {
...
export async function joinProject(projectId, userId) {
  ...
  return project.addProjectMembers([userId]).then(() => ({ success: true }));
}

export async function leaveProject(projectId, userId) {
  const project = await Post.find(projectId);
  return project.removeProjectMembers([userId]).then(() => ({ success: true }));
}

export async function createStripePaymentNotifications(
  contribution,
  creatorId
) {
...
export async function processStripeToken(userId, projectId, token, amount) {
  ...
  const projectCreator = await User.find(project.get("user_id"), {
    withRelated: "stripeAccount",
  });
  ...
  await stripe.charges.create(
    {
      ...
      description: `${contributor.get(
        "name"
      )} contributing to project ${project.get(
        "name"
    ...
    {
      stripe_account: projectCreator.relations.stripeAccount.get(
        "stripe_account_external_id"
      ),
    }
  );
  ...
  return createStripePaymentNotifications(
    contribution,
    project.get("user_id")
  ).then(() => ({ success: true }));
}
