  ...
  return {
    ...
    Post: {
      ...
      getters: {
        ...
        myEventResponse: (p) =>
          userId
            ? p
                .userEventInvitation(userId)
                .then((eventInvitation) =>
                  eventInvitation ? eventInvitation.get("response") : ""
                )
            : "",
      },
      relations: [
    ...
    Community: {
      ...
      relations: [
        ...
        },
        {
          posts: {
            ...
            filter: (
              relation,
              { search, sortBy, topic, filter, boundingBox }
            ) =>
              relation.query(
                filterAndSortPosts({
                  boundingBox,
      ...
      getters: {
        ...
        pendingInvitations: (c, { first }) =>
          InvitationService.find({ communityId: c.id, pendingOnly: true }),
        invitePath: (c) =>
          GroupMembership.hasModeratorRole(userId, c).then((isModerator) =>
            isModerator ? Frontend.Route.invitePath(c) : null
          ),
      },
    ...
    JoinRequest: {
      ...
      fetchMany: ({ communityId }) =>
        JoinRequest.where({ community_id: communityId }),
    },

    EventInvitation: {
      model: EventInvitation,
      attributes: ["response"],
      relations: [{ user: { alias: "person" } }],
    },
    ...
    Vote: {
      ...
      relations: ["post", { user: { alias: "voter" } }],
    ...
    CommunityTopic: {
      ...
      relations: ["community", { tag: { alias: "topic" } }],
      filter: nonAdminFilter((relation) =>
        relation.query((q) => {
          q.where(
      ...
      fetchMany: (args) =>
        CommunityTag.query(communityTopicFilter(userId, args)),
    },
    ...
    SavedSearch: {
      ...
      fetchMany: ({ userId }) =>
        SavedSearch.where({ user_id: userId, is_active: true }),
    },
      ...
      relations: [
        {
          communityTags: {
            ...
            filter: (
              relation,
              { autocomplete, subscribed, isDefault, visibility }
            ) =>
              relation.query(
                communityTopicFilter(userId, {
                  autocomplete,
    ...
    Activity: {
      ...
      relations: ["actor", "post", "comment", "community"],
      getters: {
    ...
    Attachment: {
      model: Media,
      attributes: ["type", "url", "thumbnail_url", "position", "created_at"],
    },
