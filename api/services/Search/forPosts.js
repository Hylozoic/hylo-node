  ...
  return Post.query((qb) => {
    ...
    if (opts.type === Post.Type.THREAD || opts.follower) {
      ...
      if (opts.type === Post.Type.THREAD) {
        ...
        qb.where("follows.user_id", opts.follower);
        qb.whereRaw(
          "(posts.user_id != ? or posts.user_id is null)",
          opts.follower
        );
      }
    }
    ...
    if (opts.type === "project" && opts.filter === "mine") {
      ...
      qb.where(function () {
        this.where("posts.user_id", opts.currentUserId).orWhere(
          "follows.user_id",
          opts.currentUserId
        );
      });
    ...
    if (opts.start_time && opts.end_time) {
      qb.whereRaw(
        "((posts.created_at between ? and ?) or (posts.updated_at between ? and ?))",
