...
export const filterAndSortPosts = curry((opts, q) => {
  ...
  const sort =
    sortColumns[sortBy] || values(sortColumns).find((v) => v === sortBy);
  if (!sort) {
  ...
  if (!type || type === "all" || type === "all+welcome") {
    ...
    q.where((q2) =>
      q2.where({ "posts.type": null }).orWhere({ "posts.type": DISCUSSION })
    );
  } else {
  ...
  if (boundingBox) {
    ...
    q.whereRaw("locations.center && ST_MakeEnvelope(?, ?, ?, ?, 4326)", [
      boundingBox[0].lng,
      boundingBox[0].lat,
      boundingBox[1].lng,
      boundingBox[1].lat,
  ...
  if (sort === "posts.updated_at" && showPinnedFirst) {
    q.orderByRaw(
      "communities_posts.pinned_at is null asc, communities_posts.pinned_at desc, posts.updated_at desc"
    );
  } else if (sort) {
    q.orderBy(sort, "desc");
  }
...
export const filterAndSortUsers = curry(
  ({ autocomplete, boundingBox, search, sortBy }, q) => {
    if (autocomplete) {
