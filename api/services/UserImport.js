...
export function createUser(attrs, options) {
  ...
  return User.isEmailUnique(email).then((unique) => {
    if (!unique) {
...
export const operations = {
  sustainableHumanIntentions: function (row, options) {
    const intention = row.Intention.trim();
    if (!intention) return false;

    const charLimit = 140;
    const limitIndex = intention.indexOf(" ", charLimit - 10);
    const sentenceEnd = intention.indexOf(". ");
    let title, details;
    ...
    return User.find(row.Email).then((user) => {
