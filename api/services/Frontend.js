...
const getSlug = function (community) {
  let slug;
  if (isString(community)) {
    // In case we passed just the slug in instead of community object
    slug = community;
  } else if (community) {
...
module.exports = {
  Route: {
    ...
    comment: function (comment, community) {
      ...
      const communitySlug = getSlug(community);

      const communityUrl = isEmpty(communitySlug)
        ? "/all"
        : `/c/${communitySlug}`;
    ...
    invitePath: function (community) {
      return `/c/${getSlug(community)}/join/${community.get(
        "beta_access_code"
      )}`;
    },
  },
