...
import {
  createS3StorageStream,
  safeBasename,
} from "../../lib/uploader/storage";

module.exports = {
  copyAsset: function (instance, type, attr) {
    const sourceUrl = instance.get(attr);
    const filename = safeBasename(sourceUrl).replace(
      /((_\d+)?(\.\w{2,4}))?$/,
      `_${Date.now()}$3`
    );

    return runPipeline(sourceUrl, filename, type, instance.id).then((url) =>
      instance.save({ [attr]: url }, { patch: true })
    );
  },
  ...
  resizeAsset: function (instance, fromAttr, toAttr, settings = {}) {
    ...
    return runPipeline(sourceUrl, filename, type, instance.id, (stream) =>
      ...
      instance.save({ [toAttr]: url }, { patch: true, transacting })
    );
  },
};
