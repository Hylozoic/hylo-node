...
const get = Promise.promisify(request.get, request);
...
API.prototype.getAllRecords = function (path, query) {
  const getPage = (page) =>
    this.get(
      path,
      Object.assign({ size: pageSize, page }, query)
...
API.prototype.getActiveContracts = function () {
  return this.getAllRecords("billing/coworkercontracts", {
...
API.prototype.fetchMembers = function () {
  ...
  return this.getSpaceInfo()
    .tap((info) => {
      this.subdomain = info.WebAddress;
    })
    .then(() =>
      this.getActiveContracts().tap((r1) => {
        contracts = r1;
      })
    )
    .then(() =>
      this.getActiveCoworkers().tap((r2) => {
        coworkers = r2;
      })
    )
    ...
    .then((records) =>
      this.options.verbose ? { contracts, coworkers, records } : records
    );
};
...
API.prototype.updateMembers = function () {
  return this.fetchMembers()
    ...
    .then((records) =>
      Promise.map(records, (r) => UserImport.createUser(r, this.options))
    )
  ...
  return this.fetchMembers()
    .then(({ contracts, coworkers, records }) =>
      Promise.map(records, (r) => {
        ...
        return User.find(r.email).then((user) => {
...
module.exports = {
  ...
  forCommunity: function (community_id, opts) {
    return NexudusAccount.where({ community_id })
      .fetch()
  ...
  updateAllCommunities: function (options) {
    return NexudusAccount.where("autoupdate", true)
      ...
      .then((accounts) =>
        Promise.map(accounts.models, (account) => {
          const { community } = account.relations;
          const api = this.forAccount(
            account,
            Object.assign({ community }, options)
          );
          return api
            .updateMembers()
