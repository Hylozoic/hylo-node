...
const generateMergeQueries = function (userId, duplicateUserId, knex) {
  const ps = [userId, duplicateUserId];
  const psp = [userId, duplicateUserId, userId];
  const updates = [];
  const push = (q, values) => updates.push(knex.raw(q, values));
  ...
  [
    ...
    const table = args[0];
    const userCol = args[1];
    const uniqueCol = args[2];
    push(
      `update ${table} set ${userCol} = ? ` +
        `where ${userCol} = ? and ${uniqueCol} not in ` +
        `(select ${uniqueCol} from ${table} where ${userCol} = ?)`,
      psp
    );
  });
...
const generateRemoveQueries = function (userId, knex) {
  const removals = [];
  const push = (q, values) => removals.push(knex.raw(q, values));
  ...
  // cascading deletes
  push(
    "delete from thanks where comment_id in " +
      "(select id from comments where user_id = ?)",
    userId
  );
  push(
    "delete from notifications where activity_id in " +
      "(select id from activities where reader_id = ?)",
    userId
  );
  push(
    "delete from notifications where activity_id in " +
      "(select id from activities where actor_id = ?)",
    userId
  );
    ...
    return bookshelf.knex
      .transaction((trx) => {
        ...
        return Promise.join(
          ...
          .spread((user, dupe) => {
            userFieldsToCopy.forEach(
              (f) => user.get(f) || user.set(f, dupe.get(f))
            );

            return (
              _.isEmpty(user.changed) ||
      ...
      .then(() =>
        queries.updates.concat(queries.deletes).map((q) => q.toSQL())
      );
  },
};
