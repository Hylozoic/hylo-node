  ...
  merge(
    {
      ...
      comments: function () {
        return this.hasMany(Comment).query((q) => {
          q.where("posts.user_id", "NOT IN", BlockedUser.blockedFor(this.id));
          q.where(function () {
            this.where("posts.type", "!=", Post.Type.THREAD).orWhere(
              "posts.type",
              null
            );
          });
      ...
      communities: function () {
        return this.queryByGroupMembership(Community).query((q) =>
          q.where("active", true)
        );
      },
      ...
      inAppNotifications: function () {
        return this.hasMany(Notification).query({
      ...
      moderatedCommunityMemberships: function () {
        return this.groupMembershipsForModel(Community).query((q) => {
          q.where("group_memberships.role", GroupMembership.Role.MODERATOR);
        });
      },

      posts: function () {
        return this.hasMany(Post).query((q) =>
          q.where(function () {
      ...
      messageThreads: function () {
        return this.followedPosts().query((q) =>
          q.where("type", Post.Type.THREAD)
        );
      },
      ...
      joinCommunity: async function (
        ...
        const memberships = await community.addGroupMembers(
          [this.id],
          {
            role,
            settings: {
      ...
      generateTokenContents: function () {
        return `crumbly:${this.id}:${this.get("email")}:${this.get(
          "created_at"
        )}`;
      },
      ...
      sendPushNotification: function (alert, url) {
        return this.devices()
          .fetch()
          .then((devices) =>
            Promise.map(devices.models, (device) =>
      ...
      hasDevice: function () {
        return this.load("devices").then(
          () => this.relations.devices.length > 0
        );
      },
      ...
      validateAndSave: function (changes) {
        ...
        return bookshelf
          .transaction((transacting) =>
            validateUserAttributes(whitelist, {
              existingUser: this,
              transacting,
            })
              ...
              .then(() =>
                Promise.all([
                  changes.password &&
                    this.setPassword(changes.password, { transacting }),
      ...
      unlinkAccount(provider) {
        ...
        return Promise.join(
          LinkedAccount.query()
            .where({ user_id: this.id, provider_key: provider })
            .del(),
      ...
      async communitiesSharedWithPost(post) {
        ...
        return intersectionBy(
          post.relations.communities.models,
          myCommunities.models,
          "id"
        );
      },
  ...
  ),
  {
    ...
    authenticate: Promise.method(function (email, password) {
      ...
      return User.query("whereRaw", "lower(email) = lower(?)", email)
        ...
        .then(function (user) {
          ...
          const account = user.relations.linkedAccounts.where({
            provider_key: "password",
          })[0];
          if (!account) {
            const keys = user.relations.linkedAccounts.pluck("provider_key");
            throw new Error(
              `password account not found. available: [${keys.join(",")}]`
            );
          }

          return compare(password, account.get("provider_user_id")).then(
            function (match) {
    ...
    find: function (id, options) {
      ...
      if (isNaN(Number(id))) {
        q = User.query((q) => {
          q.where(function () {
            this.whereRaw("lower(email) = lower(?)", id).orWhere({ name: id });
          });
        });
      } else {
        q = User.where({ id });
    ...
    encryptEmail: function (email) {
      const plaintext = process.env.MAILGUN_EMAIL_SALT + email;
      return `u=${PlayCrypto.encrypt(plaintext)}@${process.env.MAILGUN_DOMAIN}`;
    },

    decryptEmail: function (email) {
      const pattern = new RegExp(`u=(\\w+)@${process.env.MAILGUN_DOMAIN}`);
      const match = email.match(pattern);
      const hash = match[1];
      const decrypted = PlayCrypto.decrypt(hash);
      const unsalted = decrypted.replace(
        new RegExp("^" + process.env.MAILGUN_EMAIL_SALT),
        ""
      );
    ...
    resetTooltips: function (userId) {
      return User.find(userId).then((user) =>
        user.removeSetting("viewedTooltips", true)
      );
    },
    ...
    unseenThreadCount: async function (userId) {
      ...
      const lastViewed = await User.where("id", userId)
        .query()
        .select(raw("settings->'last_viewed_messages_at' as time"))
        .then((rows) => new Date(rows[0].time));

      return GroupMembership.whereUnread(userId, Post, {
        afterTime: lastViewed,
      })
        .query((q) => {
...
function validateUserAttributes(attrs, { existingUser, transacting } = {}) {
  ...
  return User.isEmailUnique(attrs.email, oldEmail, { transacting }).then(
    (unique) => unique || Promise.reject(new Error("duplicate-email"))
  );
}
