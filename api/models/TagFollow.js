...
module.exports = bookshelf.Model.extend(
  ...
  },
  {
    create(attrs, { transacting } = {}) {
      return this.forge(Object.assign({ created_at: new Date() }, attrs)).save(
        {},
        { transacting }
      );
    },
    ...
    add: function ({ tagId, userId, communityId, transacting }) {
      ...
      return TagFollow.where({
        ...
        .then(
          (follow) =>
            follow ||
            new TagFollow(attrs).save(null, { transacting }).tap(() =>
              CommunityTag.query((q) => {
                q.where("community_id", communityId);
                q.where("tag_id", tagId);
              })
                .query()
                .increment("num_followers")
