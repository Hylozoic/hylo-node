...
function updateMedia(post, type, urls, transacting) {
  if (!urls) return;
  const media = post.relations.media.filter((m) => m.get("type") === type);

  return Promise.map(media, (m) => m.destroy({ transacting })).then(() =>
...
export async function updateFollowers(post, transacting) {
  const followerIds = await post
    .followers()
    .fetch()
    .then((f) => f.pluck("id"));
  const newMentionedIds = RichText.getUserMentions(
    post.get("description")
  ).filter((id) => !followerIds.includes(id));

  return post.addFollowers(newMentionedIds, { transacting }).then((follows) => {
...
export function updateNetworkMemberships(post, transacting) {
  ...
  return post.load(["communities", "networks"], opts).then(() => {
    const newIds = compact(
      uniq(post.relations.communities.map((c) => Number(c.get("network_id"))))
    ).sort();
