...
export function findThread(userIds) {
  ...
  return Post.query((q) =>
    q.whereIn("id", subquery).where({ type: Post.Type.THREAD })
  ).fetch();
}

export default function findOrCreateThread(userId, participantIds) {
  return findThread(uniq([userId].concat(participantIds))).then(
    (post) => post || createThread(userId, uniq(participantIds))
  );
}
