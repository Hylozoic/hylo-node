...
describe("findOrCreateThread", () => {
  ...
  it("finds or creates a thread", async () => {
    ...
    const thread2 = await findOrCreateThread(u2.id, [u1.id, u2.id, u3.id]);
    expect(thread2.id).to.equal(thread.id);

    const thread3 = await findOrCreateThread(u2.id, [u2.id, u3.id]);
...
describe("validateThreadData", () => {
  let user, userSharingCommunity, userNotInCommunity, community;
  ...
  it("fails if there is a participantId for a user the creator shares no communities with", () => {
    const data = {
      participantIds: [userSharingCommunity.id, userNotInCommunity.id],
    };
    return validateThreadData(user.id, data).catch(function (e) {
      expect(e.message).to.equal(
        `no shared communities with user ${userNotInCommunity.id}`
      );
    });
  });
