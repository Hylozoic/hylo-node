...
export default function updateChildren(post, children, trx) {
  ...
  return post.load("children", { transacting: trx }).then(() => {
    const existingIds = map("id", post.relations.children.models);
    const removed = filter(
      (id) => !includes(map("id", updated), id),
      existingIds
    );
    return Promise.all([
      ...
      Promise.map(updated, (child) =>
        Post.query()
          .where("id", child.id)
          .update(omit(child, "id"))
          .transacting(trx)
      ),

      // create new requests
      some(created) &&
        Tag.find({ name: "request" }).then((tag) => {
          ...
          return Promise.map(created, (child) => {
            ...
            return Post.create(attrs, { transacting: trx }).then((post) =>
              post.tags().attach(attachment, { transacting: trx })
            );
          });
        }),
    ]);
