  ...
  Object.assign(
    {
      ...
      comments: function () {
        return this.hasMany(Comment, "comment_id").query({
          where: { active: true },
        });
      },
      ...
      createActivities: async function (trx) {
        ...
        return Activity.saveForReasons(
          newCommentActivities.concat(mentionActivities),
          trx
        );
      },
    },
  ...
  ),
  {
    ...
    createdInTimeRange: function (collection, startTime, endTime) {
      ...
      return collection.query(function (qb) {
        qb.whereRaw("comments.created_at between ? and ?", [
          startTime,
          endTime,
        ]);
    ...
    cleanEmailText: (user, text, opts = { useMarkdown: true }) => {
      ...
      lines.forEach((line, index) => {
        ...
        if (
          line.length > 0 &&
          name.startsWith(line.toLowerCase().replace(/^[- ]*/, ""))
        ) {
