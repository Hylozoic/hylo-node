...
const networkIdsQuery = function (userId) {
  ...
  return knex
    ...
    .distinct("network_id")
    .from("communities")
    .whereIn("id", communityIdsQuery)
    .whereRaw("network_id is not null");
};
...
module.exports = bookshelf.Model.extend(
  Object.assign(
    {
      ...
      communities: function () {
        return this.hasMany(Community).query({
          where: { "communities.active": true },
        });
      },
      ...
      async memberCount() {
        ...
        return GroupMembership.forIds(null, communityIds, Community)
          .query()
  ...
  ),
  {
    ...
    containsUser: function (networkId, userId) {
      ...
      return this.idsForUser(userId).then((ids) =>
        includes(ids, networkId.toString())
      );
    },
