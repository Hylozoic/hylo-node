...
module.exports = bookshelf.Model.extend(
  ...
  ),
  {
    ...
    find: function (user_id, community_id_or_slug, options) {
      ...
      const fetch = function (community_id) {
        const attrs = { user_id, community_id };
      ...
      if (isNaN(Number(community_id_or_slug))) {
        return Community.find(community_id_or_slug).then(function (community) {
    ...
    create: function (userId, communityId, opts) {
      ...
      return this.forge({
        ...
        .tap(() =>
          User.followDefaultTags(userId, communityId, opts.transacting)
        )
        .tap(() =>
          Community.find(communityId, { transacting: opts.transacting })
            .tap((community) =>
              community.save(
                {
    ...
    hasModeratorRole: function (user_id, community_id) {
      return this.find(user_id, community_id).then(
        (ms) => ms && ms.hasModeratorRole()
      );
    },
    ...
    inSameCommunity: function (userIds) {
      return this.sharedCommunityIds(userIds).then((ids) => ids.length > 0);
    },
    ...
    inSameNetwork: function (userId, otherUserId) {
      return Network.idsForUser(userId).then((ids) => {
        if (isEmpty(ids)) return false;

        return Network.idsForUser(otherUserId).then(
          (otherIds) => !isEmpty(intersection(ids, otherIds))
        );
      });
    },
    ...
    activeCommunityIds: function (user_id, moderator) {
      ...
      const query = { user_id: user_id, active: true };
      if (moderator) {
        query.role = this.MODERATOR_ROLE;
      }
      return this.query().where(query).pluck("community_id");
    },
