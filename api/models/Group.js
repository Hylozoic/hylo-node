...
module.exports = bookshelf.Model.extend(
  {
    ...
    childGroups() {
      return this.belongsToMany(Group).through(
        GroupConnection,
        "parent_group_id",
        "child_group_id"
      );
    ...
    memberships(includeInactive = false) {
      return this.hasMany(GroupMembership).query((q) =>
        includeInactive ? q : q.where("group_memberships.active", true)
      );
    },
    ...
    async updateMembers(usersOrIds, attrs, { transacting } = {}) {
      ...
      const existingMemberships = await this.memberships(true)
        .query((q) => q.where("user_id", "in", userIds))
        .fetch();
      ...
      return Promise.map(existingMemberships.models, (ms) =>
        ms.updateAndSave(updatedAttribs, { transacting })
      );
    },
    ...
    async addMembers(usersOrIds, attrs = {}, { transacting } = {}) {
      ...
      await this.updateMembers(existingUserIds, updatedAttribs, {
        transacting,
      });

      const updatedMemberships = await this.updateMembers(
        existingUserIds,
        updatedAttribs,
        { transacting }
      );
  ...
  },
  {
    ...
    async inSameGroup(userIds, typeOrModel) {
      const groupIds = await Promise.all(
        userIds.map((id) => this.pluckIdsForMember(id, typeOrModel))
      );
