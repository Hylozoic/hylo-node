...
import { filter, omitBy, some, uniq } from "lodash/fp";
...
const addToTaggable = (taggable, name, userId, opts) => {
  let association, getCommunities;
  ...
  return (
    taggable
      .load(association, opts)
      .then(findTag)
      // create the tag -- if creation fails, find the existing one
      .then(
        (tag) =>
          tag || new Tag({ name, created_at }).save({}, opts).catch(findTag)
      )
      .tap((tag) =>
        taggable
          .tags()
          ...
          .then((communityIds) => {
            ...
            const communities = filter(
              (c) => includes(communityIds, c.id),
              getCommunities(taggable).models
            );
            return Promise.map(communities, (com) =>
              Tag.addToCommunity(
                {
                  community_id: com.id,
                  tag_id: tag.id,
...
const updateForTaggable = ({ taggable, tagNames, userId, transacting }) => {
  return taggable.load("tags", { transacting }).then(() => {
    const toRemove = taggable.relations.tags.models;
    return Promise.map(toRemove, (tag) =>
      removeFromTaggable(taggable, tag, { transacting })
    ).then(() =>
      Promise.map(uniq(tagNames), (name) =>
        addToTaggable(taggable, name, userId, { transacting })
      )
    );
  });
};
...
module.exports = bookshelf.Model.extend(
  ...
  {
    addToCommunity: (
      { community_id, tag_id, user_id, description, is_default, isSubscribing },
      opts
    ) =>
      CommunityTag.where({ community_id, tag_id })
        .fetch(opts)
        .tap(
          (comTag) =>
            comTag ||
            CommunityTag.create(
              { community_id, tag_id, user_id, description, is_default },
              opts
            ).catch(() => {})
        )
        ...
        .then(
          (comTag) =>
            comTag && comTag.save({ updated_at: new Date(), is_default })
        )
        .then(
          () =>
            user_id &&
            isSubscribing &&
            TagFollow.where({ community_id, tag_id, user_id })
              .fetch(opts)
              .then(
                (follow) =>
                  follow ||
    ...
    find: function ({ id, name }, options) {
      ...
      if (name) {
        return Tag.query((qb) =>
          qb.whereRaw("lower(name) = lower(?)", name)
        ).fetch(options);
      }
    ...
    findOrCreate: function (name, options) {
      return Tag.find({ name }, options).then((tag) => {
        if (tag) return tag;
    ...
    remove: (id) => {
      return bookshelf.transaction((trx) => {
        ...
        return Promise.all(
          tables.map((t) => trx(t).where("tag_id", id).del())
    ...
    taggedPostCount: (tagId, options = {}) => {
      ...
      q.select(
        bookshelf.knex.raw("count(distinct posts_tags.post_id) AS count")
      );
    ...
    followersCount: (
      tagId,
      { userId, communityId, communitySlug, networkSlug }
    ) => {
    ...
    nonexistent: (names, communityIds) => {
      if (names.length === 0 || !communityIds || communityIds.length === 0)
        return Promise.resolve({});

      const isCommunity = (id) => (row) =>
        Number(row.community_id) === Number(id);
      const sameTag = (name) => (row) =>
        ...
        .then((rows) => {
          return names.reduce((m, n) => {
            ...
            const missing = filter(
              (id) => !some(isCommunity(id), matching),
              communityIds
            );
