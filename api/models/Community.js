...
module.exports = bookshelf.Model.extend(
  merge(
    {
      ...
      createStarterPosts: function (transacting) {
        const now = new Date();
        const timeShift = { offer: 1, request: 2, resource: 1 };
        return Community.find("starter-posts", { withRelated: ["posts"] })
          ...
          .then((posts) =>
            Promise.map(posts, (post) => {
              ...
              const time = new Date(
                now - (timeShift[post.get("type")] || 0) * 1000
              );
              return newPost
                .save({ created_at: time, updated_at: time }, { transacting })
                .then(() =>
                  Promise.all(
                    flatten([
      ...
      feedItems: function ({ first, cursor, order }) {
        return this.posts()
          ...
          .fetch()
          .then((posts) => posts.map((p) => createFeedItem({ post: p })));
      },
  ...
  ),
  {
    find: function (key, opts = {}) {
      ...
      const where = isNaN(Number(key))
        ? opts.active
          ? { slug: key, active: true }
          : { slug: key }
        : opts.active
        ? { id: key, active: true }
        : { id: key };
    ...
    copyAssets: function (opts) {
      return Community.find(opts.communityId).then((c) =>
        Promise.join(
    ...
    notifyAboutCreate: function (opts) {
      return Community.find(opts.communityId, {
        withRelated: ["creator"],
      }).then((c) => {
        const creator = c.relations.creator;
        const recipient = process.env.ASANA_NEW_COMMUNITIES_EMAIL;
    ...
    inNetworkWithUser: function (communityId, userId) {
      return Community.query()
        ...
        .pluck("network_id")
        .then(
          ([networkId]) => networkId && Network.containsUser(networkId, userId)
        );
    },
    ...
    notifySlack: function (communityId, post) {
      return Community.find(communityId).then((community) => {
        ...
        const slackMessage = Slack.textForNewPost(post, community);
    ...
    getNewAccessCode: function () {
      const test = (code) =>
        Community.where({ beta_access_code: code }).count().then(Number);
      const loop = () => {
    ...
    async create(userId, data) {
      ...
      const beta_access_code =
        attrs.beta_access_code || (await Community.getNewAccessCode());
      ...
      const memberships = await bookshelf.transaction(async (trx) => {
        ...
        return community.addGroupMembers(
          [userId],
