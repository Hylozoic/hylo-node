...
export const sendDigests = () => {
  ...
  return redis
    .getAsync(sendDigests.REDIS_TIMESTAMP_KEY)
    .then((i) => (i ? new Date(Number(i)) : fallbackTime()))
    .catch(() => fallbackTime())
    .then((time) =>
      Post.where("updated_at", ">", time).fetchAll({
        withRelated: [
          {
            comments: (q) => {
          ...
          return Promise.map(followers.models, (user) => {
            ...
            const presentComment = (comment) => {
              ...
              return comment.relations.media.length !== 0
                ? Object.assign({}, presented, {
                    image: comment.relations.media
                      .first()
                      .pick("url", "thumbnail_url"),
                  })
            ...
            if (post.get("type") === Post.Type.THREAD) {
              ...
              const otherAvatarUrls = others.map((other) =>
                other.get("avatar_url")
              );

              const participantNames =
                otherNames.slice(0, otherNames.length - 1).join(", ") +
                " & " +
              ...
              return Email.sendCommentDigest({
                ...
                data: {
                  ...
                  post_creator_avatar_url: post.relations.user.get(
                    "avatar_url"
                  ),
