  ...
  return bookshelf
    ...
    .tap((comment) =>
      Promise.all([
        ...
        isThread
          ? Queue.classMethod("Comment", "notifyAboutMessage", {
...
export const createMedia = (opts, trx) => (comment) => {
  return Promise.all(
    flatten([
      opts.attachments &&
        Promise.map(opts.attachments, (attachment, i) =>
...
function pushCommentToSockets(comment) {
  return comment.ensureLoad("user").then(() =>
    pushToSockets(
      postRoom(comment.get("post_id")),
      "commentAdded",
      Object.assign({}, refineOne(comment, ["id", "text", "created_at"]), {
        creator: refineOne(comment.relations.user, [
          "id",
          "name",
          "avatar_url",
        ]),
...
const createOrUpdateConnections = (userId, existingFollowers) => (comment) => {
  return existingFollowers
    .filter((f) => f !== userId)
    .forEach((follower) =>
      UserConnection.createOrUpdate(userId, follower, "message")
    );
};
