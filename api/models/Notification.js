...
module.exports = bookshelf.Model.extend(
  {
    ...
    send: function () {
      let action;
      return this.shouldBeBlocked().then((shouldBeBlocked) => {
        if (shouldBeBlocked) {
          this.destroy();
        ...
        switch (this.get("medium")) {
          ...
          case MEDIUM.InApp:
            ...
            action = User.incNewNotificationCount(userId).then(() =>
              this.updateUserSocketRoom(userId)
            );
            break;
        }
        if (action) {
    ...
    sendPush: function () {
      switch (
        Notification.priorityReason(this.relations.activity.get("meta").reasons)
      ) {
    ...
    sendEventInvitationPush: function () {
      ...
      if (isEmpty(communityIds))
        throw new Error("no community ids in activity");
      return Community.find(communityIds[0]).then((community) => {
        const path = url.parse(Frontend.Route.post(post, community)).path;
        const alertText = PushNotification.textForEventInvitation(post, actor);
    ...
    sendPushAnnouncement: function (version) {
      ...
      return Community.find(communityIds[0]).then((community) => {
        const path = url.parse(Frontend.Route.post(post, community)).path;
        const alertText = PushNotification.textForAnnouncement(post);
        return this.reader().sendPushNotification(alertText, path);
      });
    },
    ...
    sendPostPush: function (version) {
      ...
      return Community.find(communityIds[0]).then((community) => {
        const path = url.parse(Frontend.Route.post(post, community)).path;
        const alertText = PushNotification.textForPost(
          post,
          community,
          this.relations.activity.get("reader_id"),
          version
        );
        return this.reader().sendPushNotification(alertText, path);
      });
    ...
    sendContributionPush: function (version) {
      return this.load(["contribution", "contribution.post"]).then(() => {
        const { contribution } = this.relations.activity.relations;
        const path = url.parse(Frontend.Route.post(contribution.relations.post))
          .path;
        const alertText = PushNotification.textForContribution(
          contribution,
          version
        );
        return this.reader().sendPushNotification(alertText, path);
    ...
    sendCommentPush: function (version) {
      ...
      const path = url.parse(Frontend.Route.post(comment.relations.post)).path;
    ...
    sendJoinRequestPush: function () {
      ...
      if (isEmpty(communityIds))
        throw new Error("no community ids in activity");
      return Community.find(communityIds[0]).then((community) => {
        const path = url.parse(Frontend.Route.communityJoinRequests(community))
          .path;
        const alertText = PushNotification.textForJoinRequest(
          community,
          this.actor()
        );
        return this.reader().sendPushNotification(alertText, path);
    ...
    sendApprovedJoinRequestPush: function () {
      ...
      if (isEmpty(communityIds))
        throw new Error("no community ids in activity");
      return Community.find(communityIds[0]).then((community) => {
        const path = url.parse(Frontend.Route.community(community)).path;
        const alertText = PushNotification.textForApprovedJoinRequest(
          community,
          this.actor()
        );
        return this.reader().sendPushNotification(alertText, path);
    ...
    sendPushDonationTo: async function () {
      await this.load([
        "activity.reader",
        "activity.projectContribution",
        "activity.projectContribution.project",
        "activity.projectContribution.user",
      ]);
      const projectContribution = this.projectContribution();
      const path = url.parse(
        Frontend.Route.post(projectContribution.relations.project)
      ).path;
      const alertText = PushNotification.textForDonationTo(projectContribution);
      return this.reader().sendPushNotification(alertText, path);
    },
    ...
    sendPushDonationFrom: async function () {
      ...
      const alertText = PushNotification.textForDonationFrom(
        projectContribution
      );
      return this.reader().sendPushNotification(alertText, path);
    },
    ...
    sendAnnouncementEmail: function () {
      ...
      const description = RichText.qualifyLinks(post.get("description"));
      ...
      if (isEmpty(communityIds))
        throw new Error("no community ids in activity");
      return Community.find(communityIds[0]).then((community) =>
        reader.generateToken().then((token) =>
          Email.sendAnnouncementNotification({
            ...
            data: {
              ...
              post_user_avatar_url: Frontend.Route.tokenLogin(
                reader,
                token,
                user.get("avatar_url") +
                  "?ctt=announcement_email&cti=" +
                  reader.id
              ),
              post_user_profile_url: Frontend.Route.tokenLogin(
                reader,
                token,
                Frontend.Route.profile(user) +
                  "?ctt=announcement_email&cti=" +
                  reader.id
              ),
              post_description: description,
              post_title: decode(post.get("name")),
              post_url: Frontend.Route.tokenLogin(
                reader,
                token,
              ...
              unfollow_url: Frontend.Route.tokenLogin(
                reader,
                token,
              ...
              tracking_pixel_url: Analytics.pixelUrl("Announcement", {
                userId: reader.id,
              }),
            },
          })
    ...
    sendPostMentionEmail: function () {
      ...
      return Community.find(communityIds[0]).then((community) =>
        reader.generateToken().then((token) =>
          Email.sendPostMentionNotification({
            email: reader.get("email"),
            sender: {
              address: replyTo,
              reply_to: replyTo,
            ...
            data: {
              ...
              tracking_pixel_url: Analytics.pixelUrl("Mention in Post", {
                userId: reader.id,
              }),
            },
          })
        ...
        reader.generateToken().then((token) =>
          Email.sendNewCommentNotification({
            ...
            data: {
              ...
              commenter_profile_url: Frontend.Route.tokenLogin(
                reader,
                token,
                Frontend.Route.profile(commenter) +
                  "?ctt=comment_email&cti=" +
                  reader.id
              ),
              comment_text: text,
              post_label: postLabel,
              post_title: title,
              comment_url: Frontend.Route.tokenLogin(
                reader,
                token,
                Frontend.Route.post(post, community) +
                  "?ctt=comment_email&cti=" +
              ...
              tracking_pixel_url: Analytics.pixelUrl("Comment", {
                userId: reader.id,
              }),
            },
          })
        ...
        reader.generateToken().then((token) =>
          Email.sendJoinRequestNotification({
            ...
            data: {
              ...
              requester_profile_url: Frontend.Route.tokenLogin(
                reader,
                token,
                Frontend.Route.profile(actor) +
                  `?ctt=comment_email&cti=${reader.id}&check-join-requests=1`
              ),
              settings_url: Frontend.Route.tokenLogin(
                reader,
                token,
    ...
    sendApprovedJoinRequestEmail: function () {
      ...
      return Community.find(communityIds[0]).then((community) =>
        reader.generateToken().then((token) =>
          Email.sendApprovedJoinRequestNotification({
            email: reader.get("email"),
            sender: { name: community.get("name") },
            data: {
              community_name: community.get("name"),
              ...
              approver_profile_url: Frontend.Route.tokenLogin(
                reader,
                token,
                Frontend.Route.profile(actor) +
                  "?ctt=comment_email&cti=" +
              ...
              community_url: Frontend.Route.tokenLogin(
                reader,
                token,
    ...
    sendDonationToEmail: async function () {
      ...
      return Email.sendDonationToEmail({
        ...
        data: {
          ...
          project_url: Frontend.Route.tokenLogin(
            reader,
            token,
            Frontend.Route.post(project) +
              "?ctt=post_mention_email&cti=" +
              reader.id
          ),
          ...
          contributor_profile_url: Frontend.Route.tokenLogin(
            reader,
            token,
            Frontend.Route.profile(actor) +
              "?ctt=comment_email&cti=" +
    ...
    sendEventInvitationEmail: function () {
      ...
      return Community.find(communityIds[0]).then((community) =>
        reader.generateToken().then((token) =>
          Email.sendEventInvitationEmail({
            email: reader.get("email"),
            sender: {
              address: replyTo,
              reply_to: replyTo,
            ...
            data: {
              ...
              post_user_profile_url: Frontend.Route.tokenLogin(
                reader,
                token,
                Frontend.Route.profile(inviter) +
                  "?ctt=post_mention_email&cti=" +
                  reader.id
              ),
              post_description: description,
    ...
    shouldBeBlocked: async function () {
      ...
      const blockedUserIds = (
        await BlockedUser.blockedFor(this.get("user_id"))
      ...
      const postCreatorId = get(
        "relations.activity.relations.post.relations.user.id",
        this
      );
      const commentCreatorId = get(
        "relations.activity.relations.comment.relations.user.id",
        this
      );
      const actorId = get("relations.activity.relations.actor.id", this);

      if (
        includes(postCreatorId, blockedUserIds) ||
      ...
      const payload = {
        ...
        activity: Object.assign(
          ...
          refineOne(activity, ["created_at", "id", "meta", "unread"]),
          {
            ...
            post: refineOne(post, ["id", "name", "description"], {
              description: "details",
              name: "title",
            }),
          }
  ...
  },
  {
    ...
    findUnsent: function (options = {}) {
      ...
      return Notification.query((q) => {
        ...
        q.where(function () {
          this.where("failed_at", null).orWhere(
            "failed_at",
            "<",
            raw("now() - interval '1 hour'")
          );
        });
        q.limit(200);
      }).fetchAll(options);
    ...
    priorityReason: function (reasons) {
      ...
      const match = (label) =>
        reasons.some((r) => r.match(new RegExp("^" + label)));
