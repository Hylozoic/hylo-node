...
export async function makeGroupMemberships({
  ...
  async function processRow(row) {
    ...
    if (Array.isArray(copyColumns)) {
      for (const k of copyColumns) newRow[k] = row[k];
    } else if (copyColumns) {
      for (const k in copyColumns) newRow[copyColumns[k]] = row[k];
    }
    return newRow;
  }
...
export async function deactivateMembershipsByGroupDataType(group_data_type) {
  const parents = await Group.where({
    group_data_type,
    active: false,
  }).fetchAll({ withRelated: "memberships" });
  const setInactive = (group) =>
    Promise.map(group.relations.memberships.models, (membership) =>
      membership.save({ active: false })
    );
...
export async function updateGroupMemberships({
  ...
  const rows = await model
    .query()
    .select(["user_id", foreignKey, ...selectColumns]);
...
export async function makeGroupConnectionsM2M({
  model,
  filter,
  child,
  parent,
}) {
  const { foreignKey: childFk, target: childModel } = getRelatedData(
    model,
    child
  );
  const { foreignKey: parentFk, target: parentModel } = getRelatedData(
    model,
    parent
  );
...
async function seed() {
  ...
  */
  ...
  console.log(
    "Community.network_id:",
    await makeGroupConnectionsFk({
      model: Community,
      parent: "network",
    })
  );
