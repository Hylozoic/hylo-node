...
module.exports = bookshelf.Model.extend(
  {
    ...
    updatePassword: function (password, { transacting } = {}) {
      return hash(password, 10).then((provider_user_id) =>
  ...
  },
  {
    create: function (
      userId,
      { type, profile, password, token },
      { transacting, updateUser } = {}
    ) {
      return (() =>
        type === "password" ? hash(password, 10) : Promise.resolve(null))()
        .then((hashed) =>
          new LinkedAccount({
            provider_key: type,
    ...
    updateUser: function (userId, { type, profile, transacting } = {}) {
      return User.find(userId, { transacting }).then((user) => {
