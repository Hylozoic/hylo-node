...
const commentersQuery = (limit, post, currentUserId) => (q) => {
  ...
  if (currentUserId) {
    ...
    q.orderBy(
      bookshelf.knex.raw(
        `case when user_id = ${currentUserId} then -1 else user_id end`
      )
    );
  }
...
module.exports = bookshelf.Model.extend(
  Object.assign(
    {
      ...
      children: function () {
        return this.hasMany(Post, "parent_post_id").query({
          where: { active: true },
        });
      },
      ...
      getTagsInComments: function (opts) {
        ...
        return this.load("comments.tags", opts).then(() =>
          uniqBy(
            "id",
            flatten(this.relations.comments.map((c) => c.relations.tags.models))
          )
        );
      },
      ...
      getCommenters: function (first, currentUserId) {
        return User.query(
          commentersQuery(first, this, currentUserId)
        ).fetchAll();
      },

      getCommentersTotal: function (currentUserId) {
        return countTotal(
          User.query(commentersQuery(null, this, currentUserId)).query(),
          "users"
        ).then((result) => {
          if (isEmpty(result)) {
            return 0;
      ...
      removeFollowers: async function (userIds, opts) {
        return this.updateGroupMembers(
          userIds,
      ...
      unreadCountForUser: function (userId) {
        return this.lastReadAtForUser(userId).then((date) => {
          if (date > this.get("updated_at")) return 0;
          return Aggregate.count(
            this.comments().query((q) => q.where("created_at", ">", date))
          );
        });
      },
      ...
      async lastReadAtForUser(userId) {
        const user = await this.groupMembersWithPivots()
          .query((q) => q.where("users.id", userId))
          .fetchOne();
        return new Date((user && user.pivot.getSetting("lastReadAt")) || 0);
      },

      pushTypingToSockets: function (
        userId,
        userName,
        isTyping,
        socketToExclude
      ...
      createActivities: async function (trx) {
        ...
        let members = await Promise.all(
          communities.map(async (community) => {
            ...
            const isModerator = await GroupMembership.hasModeratorRole(
              this.get("user_id"),
              community
            );
        ...
        const readers = filter(
          (r) => r.reader_id !== this.get("user_id"),
        ...
        return this.votes()
          ...
          .then((vote) =>
            bookshelf.transaction((trx) => {
              const inc = (delta) => () =>
                this.save(
                  { num_votes: this.get("num_votes") + delta },
                  { transacting: trx }
                );
      ...
      removeFromCommunity: function (idOrSlug) {
        return PostMembership.find(this.id, idOrSlug).then((membership) =>
          membership.destroy()
        );
      },
      ...
      getNewPostSocketPayload: function () {
        ...
        return Object.assign(
          ...
          ),
          {
            ...
            creator,
            linkPreview: refineOne(linkPreview, [
              "id",
              "image_url",
              "title",
              "url",
      ...
      totalContributions: async function () {
        ...
        return this.relations.projectContributions.models.reduce(
          (total, contribution) => total + contribution.get("amount"),
          0
        );
      },
    },
  ...
  ),
  {
    ...
    create: function (attrs, opts) {
      return Post.forge(_.merge(Post.newPostAttrs(), attrs)).save(
        null,
        _.pick(opts, "transacting")
      );
    },
    ...
    async updateFromNewComment({ postId, commentId }) {
      ...
      return Promise.all([
        ...
        commentId &&
          Group.whereIdAndType(postId, Post)
            .query()
        ...
        commentId &&
          Comment.where("id", commentId)
            ...
            .pluck("user_id")
            .then(([userId]) =>
              Post.find(postId).then((post) => post.markAsRead(userId))
            ),
      ]);
    },
    ...
    createActivities: (opts) =>
      Post.find(opts.postId).then(
        (post) =>
          post && bookshelf.transaction((trx) => post.createActivities(trx))
      ),
        ...
        Tag.where("name", "in", ["request", "offer", "resource", "intention"])
          ...
          .then((tags) =>
            Post.query((q) => {
              ...
              .then((posts) =>
                Promise.each(posts.models, (post) => {
                  const untype = () =>
                    post.save({ type: null }, { patch: true, transacting });
                  ...
                  return post
                    .selectedTags()
                    .attach(tags.find(matches).id, { transacting })
                    .then(untype);
                })
              )
    ...
    notifySlack: ({ postId }) =>
      Post.find(postId, {
        withRelated: ["communities", "user", "relatedUsers"],
      }).then((post) => {
        if (!post) return;
        const slackCommunities = post.relations.communities.filter((c) =>
          c.get("slack_hook_url")
        );
        return Promise.map(slackCommunities, (c) =>
          Community.notifySlack(c.id, post)
        );
      }),
  }
);
