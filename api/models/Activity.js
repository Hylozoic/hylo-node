...
const removeForRelation = (model) => (id, trx) => {
  ...
  return Activity.where(`${model}_id`, id)
    .query()
    .pluck("id")
    .transacting(trx)
    .then((ids) => {
      ...
      return Notification.where("activity_id", "in", ids)
        .destroy(trxOpt)
...
module.exports = bookshelf.Model.extend(
  ...
  },
  {
    ...
    filterInactiveContent: (q) => {
      q.whereRaw("(comments.active = true or comments.id is null)").leftJoin(
        "comments",
        function () {
      ...
      q.whereRaw("(posts.active = true or posts.id is null)").leftJoin(
        "posts",
        function () {
    ...
    joinWithCommunity: (communityId, q) => {
      q.where("communities_posts.community_id", communityId).join(
        "communities_posts",
        function () {
          this.on("comments.post_id", "communities_posts.post_id").orOn(
            "posts.id",
    ...
    unreadCountForUser: function (user) {
      return Activity.query()
        .where({ reader_id: user.id, unread: true })
        .count()
    ...
    saveForReasons: function (activities, trx) {
      return Promise.map(mergeByReader(activities), (activity) => {
        const attrs = Object.assign({}, omit(activity, "reasons"), {
    ...
    communityIds: function (activity) {
      if (activity.get("post_id")) {
        return get(activity, "relations.post.relations.communities", []).map(
          (c) => c.id
        );
      } else if (activity.get("comment_id")) {
        return get(
          activity,
          "relations.comment.relations.post.relations.communities",
          []
        ).map((c) => c.id);
    ...
    generateNotificationMedia: async function (activity) {
      ...
      const notifications = [];
      const communities = Activity.communityIds(activity);

      const user = activity.relations.reader;

      const memberships = await user
        .groupMembershipsForModel(Community)
      ...
      if (
        (!isEmpty(emailable) && !isJustNewPost(activity)) ||
