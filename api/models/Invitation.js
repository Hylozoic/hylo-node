...
module.exports = bookshelf.Model.extend(
  Object.assign(
    {
      ...
      async use(userId, { transacting } = {}) {
        ...
        const membership =
          (await GroupMembership.forPair(user, community).fetch({
            transacting,
        ...
        if (!this.isUsed()) {
          await this.save(
            { used_by_id: userId, used_at: new Date() },
      ...
      expire: function (userId, opts = {}) {
        ...
        return this.save(
          { expired_by_id: userId, expired_at: new Date() },
          { patch: true, transacting }
        );
      },

      send: function () {
        return this.ensureLoad(["creator", "community", "tag"]).then(() => {
  ...
  ),
  {
    ...
    createAndSend: function ({ invitation }) {
      return Invitation.find(invitation.id).then((invitation) =>
        invitation.send()
      );
    },
    ...
    resendAllReady() {
      return Invitation.query((q) => {
        const whereClause =
          "((sent_count=1 and last_sent_at < now() - interval '4 day') or " +
