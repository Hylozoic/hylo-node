...
module.exports = bookshelf.Model.extend(
  {
    ...
    send: async function (options) {
      ...
      const disabled =
        ...
        (!process.env.PUSH_NOTIFICATIONS_TESTING_ENABLED ||
          !device.get("tester"));
  ...
  },
  {
    textForContribution: function (contribution) {
      ...
      return `You have been added as a contributor to the request "${post.get(
        "name"
      )}"`;
    },
    ...
    textForApprovedJoinRequest: function (community, actor) {
      return `${actor.get(
        "name"
      )} approved your request to join ${community.get("name")}`;
    },
