...
const findUser = function (service, email, id) {
  return User.query(function (qb) {
    ...
    qb.where(function () {
      this.where({
        provider_user_id: id,
        "linked_account.provider_key": service,
    ...
    .then((users) => {
      ...
      if (users.length >= 2) {
        return users.find((u) =>
          _.some(
            u.relations.linkedAccounts.models,
            (a) =>
              a.get("provider_user_id") === id &&
              a.get("provider_key") === service
          )
        );
      }
      return users.first();
...
const upsertUser = (req, service, profile) => {
  return findUser(service, profile.email, profile.id).then((user) => {
    if (user) {
      return (
  ...
  return LinkedAccount.where({
    ...
    .then((account) => {
      if (account) {
        ...
        return account
          .save({ user_id: userId }, { patch: true })
          .then(() =>
            LinkedAccount.updateUser(userId, { type: service, profile })
          );
      }
...
const finishOAuth = function (strategy, req, res, next) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const authCallback = function (err, profile, info) {
      ...
      return (UserSession.isLoggedIn(req) ? upsertLinkedAccount : upsertUser)(
        req,
        provider,
        profile
      )
        .then(() =>
          UserExternalData.store(req.session.userId, provider, profile._json)
        )
        .then(() => respond())
        .catch(respond);
    };
