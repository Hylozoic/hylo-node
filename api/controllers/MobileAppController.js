...
module.exports = {
  updateInfo: function (req, res) {
    const result = {
      ...
      message:
        "The version you are using is no longer compatible with the site. Please go to the App Store now to update",
  ...
  logError: function (req, res) {
    ...
    rollbar.error(new Error("ReactNativeError"), null, {
...
function shouldUpdate(version, platform) {
  ...
  if (semver.valid(version)) {
    if (
      semver.lt(
        version,
        process.env.MINIMUM_SUPPORTED_MOBILE_VERSION || "0.0.0"
      )
    ) {
...
function resultBuilder(type, platform) {
  const appStoreLink = process.env.IOS_APP_STORE_URL;
  const playStoreLink = process.env.ANDROID_APP_STORE_URL;
  const title =
    type === "suggest"
      ? "An update is available"
      : "A new version of the app is available";
  const store = platform === "ios" ? "App Store" : "Play Store";
  const suggestUpdateMessage = `The version you are using is no longer up to date. Please go to the ${store} to update.`;
  const forceUpdateMessage = `The version you are using is no longer supported. Please go to the ${store} now to update.`;
  const message =
    type === "suggest" ? suggestUpdateMessage : forceUpdateMessage;
  const link = platform === "ios" ? appStoreLink : playStoreLink;
  return {
    type,
    title,
    message,
