...
const PostController = {
  createFromEmailForm: function (req, res) {
    ...
    let community;
    return Post.where({ name: attributes.name, user_id: userId })
      .fetch()
      .then((post) => {
      ...
      .then(
        (stop) =>
          stop ||
          Community.find(communityId).then((c) => {
            community = c;
            if (!c.get("active")) {
              const message =
                "Your post was not created. That community no longer exists.";
              res.redirect(
                Frontend.Route.root() +
                  `?notification=${encodeURIComponent(message)}&error=1`
              );
              return true;
            }
          })
  ...
  typing: function (req, res) {
    ...
    return User.find(req.session.userId)
      .then((user) =>
        post.pushTypingToSockets(user.id, user.get("name"), isTyping, socket)
      )
      .then(() => res.ok({}));
  },
