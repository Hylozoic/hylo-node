...
module.exports = {
  createFromEmail: function (req, res) {
    ...
    return Promise.join(
      ...
      (post, user) => {
        if (!post)
          return res.status(422).send("valid token, but post not found");
        if (!user)
          return res.status(422).send("valid token, but user not found");
        ...
        return createComment(replyData.userId, {
          text,
          post,
          created_from: "email",
  ...
  createBatchFromEmailForm: function (req, res) {
    ...
    return Community.find(communityId).then((community) =>
      Promise.map(postIds, (id) => {
        if (isEmpty(replyText(id))) return;
        return Post.find(id, { withRelated: ["communities"] }).then((post) => {
          if (
            !post ||
            !includes(communityId, post.relations.communities.pluck("id"))
          ) {
            failures = true;
