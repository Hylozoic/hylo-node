...
exports.seed = (knex, Promise) =>
  warning()
    ...
    .then(() =>
      knex("communities").insert({ name: community, slug: communitySlug })
    )
...
function addUsersToCommunities(knex, Promise) {
  ...
  return knex("users")
    .select("id")
    .then((users) =>
      Promise.all(users.map(({ id }) => fakeMembership(id, knex)))
    );
}
...
function addPostsToCommunities(knex, Promise) {
  ...
  return knex("posts")
    ...
    .then((posts) =>
      Promise.all(
        posts.map(({ post_id, user_id }) =>
          knex("communities_users")
            .where("communities_users.user_id", user_id)
            .first("community_id")
            .then(({ community_id }) =>
              knex("communities_posts").insert({ post_id, community_id })
            )
        )
      )
    );
...
function createThreads(knex, Promise) {
  ...
  return knex("communities")
    ...
    .then((community) =>
      Promise.all(
        [...new Array(n.threads)].map(() =>
          fakeThread(community.id, knex, Promise)
        )
      )
    );
}
  ...
  return knex("follows")
    ...
    .then((follows) => {
      ...
      return Promise.all(
        ...
        [...follows, ...follows, ...follows, ...follows, ...follows].map(
          (follow) => {
...
function sample(entity, where, knex, limit = 1) {
  return knex(entity).where(where).select().orderByRaw("random()").limit(limit);
}
...
function fakeMembership(user_id, knex) {
  return sample("communities", true, knex).then(([community]) =>
    knex("communities_users").insert({
      active: true,
      community_id: community.id,
      created_at: knex.fn.now(),
...
function fakePost(knex, Promise) {
  return sample("users", true, knex).then(([user]) => ({
